...
# Engine FFI ABI for plugin lifecycle.
p = root / 'dart/digitor_engine_ffi/lib/src/native_production_api.dart'
s = p.read_text()
if 'DigitorFlutterProductionPluginAttachmentNative' not in s:
    anchor = "@Native<Int32 Function()>(symbol: 'digitor_flutter_production_host_registered')\nexternal int digitorFlutterProductionHostRegistered();\n"
    addition = anchor + """
...
@Native<Int32 Function(Pointer<DigitorFlutterProductionPluginAttachmentNative>)>(
...
"""
    if anchor not in s:
        fail('native production API anchor missing')
    p.write_text(s.replace(anchor, addition, 1))

# Flutter platform bridge exposes one opaque stable native token.
p = root / 'dart/digitor_engine_ffi/lib/src/platform_host.dart'
s = p.read_text()
if 'Future<int> productionRegistrarToken()' not in s:
    anchor = '  Future<void> close() async {\n'
    addition = """  /// Returns the stable native registrar token owned by the platform plugin.
  ///
...
""" + anchor
    if anchor not in s:
        fail('platform host close anchor missing')
    p.write_text(s.replace(anchor, addition, 1))

# Bootstrap runs after the engine native asset is loaded and owns detach token.
(root / 'dart/digitor_engine_ffi/lib/src/app_bootstrap.dart').write_text("""import 'dart:ffi';
    ...
    try {
      ...
      if (textureReady &&
        ...
        if (platformCode == 0) {
          ...
          final identity =
              'flutter.${capabilities.platform}.production-host.v1'.toNativeUtf8();
          try {
            attachment.ref
              ..structSize =
              ...
              ..flutterTextureRegistrar = Pointer<Void>.fromAddress(registrarToken)
              ..implementationIdentity = identity;
            registrationResult = digitorFlutterProductionPluginAttach(attachment);
          } finally {
...
# Workspace initializes/loads engine first; registration follows afterwards.
p = root / 'dart/digitor_engine_ffi/lib/src/editor_controller.dart'
s = p.read_text()
old = """    final productionBootstrap =
...
"""
new = """    final workspace = await DigitorEditorWorkspace.create(
      preferredBackend: preferredBackend,
      allowCpuFallback: allowCpuFallback,
      sampleRate: sampleRate,
      channels: channels,
...
"""
if old not in s:
    fail('editor controller create anchor missing')
...
"""
if old not in s:
    fail('editor controller close anchor missing')
s = s.replace(old, new, 1)
old = """    _disposed = true;
...
"""
if old not in s:
    fail('editor controller dispose anchor missing')
p.write_text(s.replace(old, new, 1))

# Windows: Flutter texture registrar pointer is stable for plugin lifetime.
p = root / 'dart/digitor_engine_ffi/windows/digitor_engine_ffi_plugin.cpp'
s = p.read_text()
if 'productionRegistrarToken' not in s:
    anchor = '  const auto* args = Arguments(call);\n'
    addition = """  if (call.method_name() == \"productionRegistrarToken\") {
...
""" + anchor
    if anchor not in s:
        fail('windows method-call arguments anchor missing')
    p.write_text(s.replace(anchor, addition, 1))

# Android: native bridge owns a process-stable token address.
p = root / 'dart/digitor_engine_ffi/android/src/main/cpp/platform_host_jni.cpp'
s = p.read_text()
if 'nativeProductionRegistrarToken' not in s:
    s += """
...
p = root / 'dart/digitor_engine_ffi/android/src/main/kotlin/com/primedigitor/digitor_engine_ffi/DigitorEngineFfiPlugin.kt'
s = p.read_text()
if 'nativeProductionRegistrarToken' not in s:
    anchor = '    private external fun nativeReleaseWindow(handle: Long)\n\n'
    if anchor not in s:
        fail('android native method anchor missing')
    s = s.replace(anchor, anchor + '    private external fun nativeProductionRegistrarToken(): Long\n\n', 1)
if '"productionRegistrarToken" ->' not in s:
    anchor = '            "createTexture" -> createTexture(call, result)\n'
    addition = """            \"productionRegistrarToken\" -> {
...
""" + anchor
    if anchor not in s:
        fail('android method switch anchor missing')
    s = s.replace(anchor, addition, 1)
p.write_text(s)

# Apple: texture registry object identity is stable for plugin lifetime.
for rel in [
    'dart/digitor_engine_ffi/ios/Classes/DigitorEngineFfiPlugin.mm',
    'dart/digitor_engine_ffi/macos/Classes/DigitorEngineFfiPlugin.mm',
]:
    p = root / rel
    s = p.read_text()
    if 'productionRegistrarToken' in s:
        continue
    anchor = '  NSDictionary *args = [call.arguments isKindOfClass:[NSDictionary class]] ? call.arguments : nil;\n'
    addition = """  if ([call.method isEqualToString:@\"productionRegistrarToken\"]) {
    if (!self.textures) {
...
""" + anchor
    if anchor not in s:
        fail(f'apple method-call arguments anchor missing: {rel}')
    p.write_text(s.replace(anchor, addition, 1))

# Public platform-host contract coverage.
p = root / 'dart/digitor_engine_ffi/test/platform_host_test.dart'
s = p.read_text()
if "production registrar token is opaque and non-zero" not in s:
    test = """
...
"""
    idx = s.rfind('\n}')
    if idx < 0:
        fail('platform host test closing anchor missing')
    p.write_text(s[:idx] + test + s[idx:])
