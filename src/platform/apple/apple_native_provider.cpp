...
[[nodiscard]] bool platform_supported(ProductionPlatform platform) noexcept {
...
[[nodiscard]] bool capabilities_valid(
  ...
  return value.metal && value.iosurface && value.core_video &&
         value.video_toolbox && value.hardware_encoder &&
         value.flutter_texture_bridge && value.cvmetaltexture_cache &&
         value.metal_completion_sync && value.iosurface_pixel_buffer_pool &&
         value.color_attachments && value.zero_cpu_readback &&
         value.zero_cpu_staging && !value.device_identity.empty() &&
...
[[nodiscard]] const char* prefix(ProductionPlatform platform) noexcept {
...
AppleNativeProviderResult create_apple_native_provider(
  ...
  return out;
#else
  if (!platform_supported(bindings.platform)) {
  ...
  const std::string platform_prefix = prefix(bindings.platform);
  NativePlatformProvider provider{};
  provider.platform = bindings.platform;
  provider.timeline = evidence(platform_prefix + ".timeline." +
                               bindings.capabilities.device_identity);
  provider.flutter_texture = evidence(platform_prefix + ".flutter." +
                                      bindings.flutter.implementation_identity);
  provider.encoder = evidence(platform_prefix + ".encoder." +
