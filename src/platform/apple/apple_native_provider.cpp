...
[[nodiscard]] bool capabilities_valid(
  ...
  return value.metal && value.video_toolbox && value.core_video &&
         value.core_media && value.iosurface && value.cvmetal_texture_cache &&
         value.hardware_encoder && value.iosurface_pixel_buffer_pool &&
         value.flutter_texture_bridge && value.metal_completion_sync &&
         value.lifecycle_safe && value.hdr_attachments &&
         value.zero_cpu_readback && value.zero_cpu_staging &&
         !value.device_identity.empty() && !value.encoder_identity.empty();
}
...
AppleNativeProviderResult create_apple_native_provider(
  ...
  if (bindings.platform != ProductionPlatform::macos &&
      bindings.platform != ProductionPlatform::ios) {
    out.result = DIGITOR_RESULT_INVALID_ARGUMENT;
    out.diagnostic = "Apple provider requires macOS or iOS platform identity";
    return out;
  }
  ...
  const auto prefix = bindings.platform == ProductionPlatform::macos
                          ? std::string("macos")
                          : std::string("ios");
  provider.timeline = evidence(prefix + ".timeline." +
                               bindings.capabilities.device_identity);
  provider.flutter_texture = evidence(prefix + ".flutter." +
                                      bindings.flutter.implementation_identity);
  provider.encoder = evidence(prefix + ".encoder." +
