...
[[nodiscard]] bool capabilities_valid(
  ...
  return value.metal && value.iosurface && value.core_video &&
         ...
         value.hdr_attachments && value.zero_cpu_readback &&
...
AppleNativeProviderResult create_apple_native_provider(
  ...
  provider.timeline = evidence(std::string(platform_name) + ".timeline." +
                               bindings.capabilities.device_identity);
  provider.flutter_texture = evidence(std::string(platform_name) + ".flutter." +
                                      bindings.flutter.implementation_identity);
  provider.encoder = evidence(std::string(platform_name) + ".encoder." +
