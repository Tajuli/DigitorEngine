...
[[nodiscard]] NativeImplementationEvidence evidence(const std::string& id) {
...
[[nodiscard]] bool capabilities_valid(
    const AppleNativeProviderCapabilities& c) noexcept {
  return c.metal && c.iosurface && c.cvpixelbuffer &&
         c.cvmetaltexture_cache && c.videotoolbox && c.hardware_encoder &&
         c.flutter_texture_bridge && c.metal_completion_sync &&
         c.color_attachments && c.hdr_attachments && c.zero_cpu_readback &&
