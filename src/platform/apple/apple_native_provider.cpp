...
bool capabilities_valid(const AppleNativeProviderCapabilities& c) noexcept {
  return c.metal && c.iosurface && c.cvpixelbuffer && c.cvmetaltexture_cache &&
         c.video_toolbox && c.hardware_encoder && c.flutter_texture_bridge &&
         c.color_attachments && c.hdr_attachments && c.lifecycle_safe &&
...
AppleNativeProviderResult create_apple_native_provider(
  ...
  const std::string prefix =
      bindings.platform == ProductionPlatform::macos ? "macos" : "ios";
  provider.timeline = evidence(prefix + ".timeline." + bindings.capabilities.device_identity);
  provider.flutter_texture = evidence(prefix + ".flutter." + bindings.flutter.implementation_identity);
