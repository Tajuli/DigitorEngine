...
VkFormat to_vk_format(NativeEffectFormat format) noexcept {
  switch (format) {
...
struct State final {
  ...
  bool initialize(std::string& diagnostic) {
    ...
    if (vkAllocateCommandBuffers(device, &allocation, &command_buffer) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect command buffer allocation failed";
      return false;
    }
    VkFenceCreateInfo fence_info{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
    if (vkCreateFence(device, &fence_info, nullptr, &fence) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect fence creation failed";
      return false;
    }
    return true;
  }
  ...
  bool begin(std::string& diagnostic) {
    ...
    if (vkResetCommandBuffer(command_buffer, 0) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect command buffer reset failed";
      return false;
    }
    VkCommandBufferBeginInfo begin_info{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
    begin_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    if (vkBeginCommandBuffer(command_buffer, &begin_info) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect command buffer begin failed";
      return false;
    }
    recording = true;
    return true;
  ...
  bool submit(std::string& diagnostic) {
    std::lock_guard<std::mutex> lock(mutex);
    if (!recording) {
      diagnostic = "Windows Vulkan effect submission has no recorded passes";
      return false;
    }
    if (vkEndCommandBuffer(command_buffer) != VK_SUCCESS) {
      recording = false;
      diagnostic = "Windows Vulkan effect command buffer end failed";
      return false;
    }
    recording = false;
    if (vkResetFences(device, 1, &fence) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect fence reset failed";
      return false;
    }
    ...
    if (vkQueueSubmit(queue, 1, &submit_info, fence) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect queue submission failed";
      return false;
    }
    const VkResult wait = vkWaitForFences(device, 1, &fence, VK_TRUE, 30'000'000'000ull);
    if (wait != VK_SUCCESS) {
      diagnostic = wait == VK_TIMEOUT
          ? "Windows Vulkan effect GPU completion timed out"
          : "Windows Vulkan effect GPU completion failed";
      return false;
    }
    return true;
  }
...
WindowsVulkanEffectProviderResult create_windows_vulkan_effect_provider(
    WindowsVulkanEffectProviderBindings bindings) noexcept {
  WindowsVulkanEffectProviderResult out{};
  if (!bindings.physical_device || !bindings.device || !bindings.queue ||
    ...
    out.diagnostic = "Windows Vulkan effect provider bindings are incomplete";
    return out;
  }
  if (!bindings.supports_external_memory ||
    ...
    out.diagnostic = "Windows Vulkan effects require external-memory and synchronization support";
    return out;
  }
  ...
  provider.allocate_transient = [state](const NativeEffectSurface& prototype,
                                        ...
                                        std::string& diagnostic) {
    const VkFormat format = to_vk_format(prototype.format);
    if (format == VK_FORMAT_UNDEFINED) {
      diagnostic = "unsupported Windows Vulkan effect transient format";
      return false;
    }
    ...
    if (vkCreateImage(state->device, &image_info, nullptr, &owned->image) != VK_SUCCESS) {
      diagnostic = "Windows Vulkan effect transient image creation failed";
      return false;
    }
    VkMemoryRequirements requirements{};
    vkGetImageMemoryRequirements(state->device, owned->image, &requirements);
    const auto type = find_memory_type(state->physical_device,
                                       requirements.memoryTypeBits,
                                       VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (type == UINT32_MAX) {
      vkDestroyImage(state->device, owned->image, nullptr);
      diagnostic = "Windows Vulkan effect device-local memory is unavailable";
      return false;
    }
    ...
    if (vkAllocateMemory(state->device, &allocation, nullptr, &owned->memory) != VK_SUCCESS ||
      ...
      vkDestroyImage(state->device, owned->image, nullptr);
      diagnostic = "Windows Vulkan effect transient memory allocation failed";
      return false;
    }
    output = prototype;
  ...
  provider.record_pass = [state](const NativeEffectPass& pass,
    ...
    if (!input || !output || input == output) {
      diagnostic = "invalid Windows Vulkan effect pass images";
      state->abort();
      return false;
    }
  ...
  provider.submit = [state](std::string& diagnostic) {
    return state->submit(diagnostic);
  };
...
namespace digitor {
WindowsVulkanEffectProviderResult create_windows_vulkan_effect_provider(
    WindowsVulkanEffectProviderBindings) noexcept {
  WindowsVulkanEffectProviderResult out{};
  out.result = DIGITOR_RESULT_UNSUPPORTED;
  out.diagnostic = "Windows Vulkan effect provider is only available on Windows";
  return out;
}
}  // namespace digitor
