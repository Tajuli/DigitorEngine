...
void update_average(double& average, std::uint64_t count, double sample) noexcept {
  ...
  average += (sample - average) / static_cast<double>(count + 1);
}
}  // namespace
...
bool ProductionPlaybackEngine::frame_is_acceptable(const ProductionPlaybackFrame& frame) const noexcept {
  ...
  if (config_.require_gpu_frames && frame.frame->backend() == DIGITOR_RENDERER_CPU) return false;
  return true;
}
...
void ProductionPlaybackEngine::worker_loop(std::stop_token token) {
  while (!token.stop_requested()) {
    ...
    std::int64_t decode_pts;
    {
      ...
      decode_pts = std::max(next_decode_pts_us_, requested_position_us_);
      next_decode_pts_us_ = decode_pts + frame_interval_us();
    }
...
DigitorResult ProductionPlaybackEngine::tick(std::int64_t now_us) {
  const auto position = transport_.position_us(now_us);
  ...
  if (selected->pts_us + selected->duration_us + config_.late_frame_threshold_us < position) {
    std::lock_guard lock(mutex_);
    ++counters_.dropped_frames;
