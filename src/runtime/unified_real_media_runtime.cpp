...
UnifiedRealMediaRuntime::UnifiedRealMediaRuntime(
  ...
  playback_ = std::make_unique<ProductionPlaybackEngine>(
      ...
      [this](const ProductionPlaybackFrame& frame) { return present_for_flutter(frame); });
}
...
std::optional<ProductionPlaybackFrame> UnifiedRealMediaRuntime::decode_for_playback(
  ...
  const auto result = decode_->decode(*frame_number, decoded, &diagnostic);
...
DigitorResult UnifiedRealMediaRuntime::present_for_flutter(
  ...
  try { result = presenter_(frame.frame, descriptor); }
  catch (const std::bad_alloc&) { result = DIGITOR_RESULT_OUT_OF_MEMORY; }
  catch (...) { result = DIGITOR_RESULT_INTERNAL_ERROR; }
  if (result == DIGITOR_RESULT_OK) {
    std::scoped_lock lock(mutex_);
    last_texture_ = descriptor;
  }
  return result;
}
...
void UnifiedRealMediaRuntime::stop() { playback_->stop(); }
DigitorResult UnifiedRealMediaRuntime::seek(std::int64_t pts, std::int64_t now,
  ...
  if (result == DIGITOR_RESULT_OK) playback_->seek(pts, now);
  return result;
}
DigitorResult UnifiedRealMediaRuntime::scrub(std::int64_t pts, std::int64_t now,
                                             std::string* diagnostic) noexcept {
  const auto result = decode_->seek(pts, diagnostic);
  if (result == DIGITOR_RESULT_OK) playback_->scrub(pts, now);
  return result;
}
void UnifiedRealMediaRuntime::step_frames(std::int64_t count, std::int64_t now) { playback_->step_frames(count, now); }
bool UnifiedRealMediaRuntime::set_rate(double rate, std::int64_t now) { return playback_->set_rate(rate, now); }
DigitorResult UnifiedRealMediaRuntime::tick(std::int64_t now) { return playback_->tick(now); }
std::int64_t UnifiedRealMediaRuntime::update_audio_clock(std::int64_t audio, std::int64_t now) { return playback_->update_audio_clock(audio, now); }
void UnifiedRealMediaRuntime::set_memory_pressure(PlaybackPressure pressure) { playback_->set_memory_pressure(pressure); }
void UnifiedRealMediaRuntime::set_thermal_pressure(PlaybackPressure pressure) { playback_->set_thermal_pressure(pressure); }
void UnifiedRealMediaRuntime::set_proxy_available(bool available) noexcept { playback_->set_proxy_available(available); }
ProductionPlaybackTelemetry UnifiedRealMediaRuntime::telemetry(std::int64_t now) const { return playback_->telemetry(now); }
HardwareDecodeQualification UnifiedRealMediaRuntime::decode_qualification() const { return decode_->qualification(); }
