...
DigitorResult ProductionMediaGraphRuntime::render_frame_at_timestamp(
  ...
  ProductionDecodedFrame decoded;
  std::string decode_diagnostic;
  result = decoder_->decode_at_timestamp(timestamp_us, decoded, &decode_diagnostic);
  if (result != DIGITOR_RESULT_OK) {
    set_diagnostic(diagnostic, decode_diagnostic.empty()
        ? "production hardware decode failed" : std::move(decode_diagnostic));
    return result;
...
DigitorResult ProductionMediaGraphRuntime::preview_at_timestamp(
    std::int64_t timestamp_us, ProcessedGpuFramePtr* out_frame,
    std::string* diagnostic) noexcept {
  try {
    if (!presenter_) {
      set_diagnostic(diagnostic, "native Flutter preview presenter is not initialized");
    ...
    RenderedFrame rendered;
    const auto result = render_frame_at_timestamp(timestamp_us, rendered, diagnostic);
    if (result != DIGITOR_RESULT_OK) return result;
