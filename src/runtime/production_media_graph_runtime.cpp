...
DigitorResult ProductionMediaGraphRuntime::validate_graph(
  ...
  try {
    if (graph_->recipe_identity() != graph_identity_) {
      set_diagnostic(
          diagnostic,
          "production node graph changed after binding; create/rebind the "
          "production runtime so preview and export use one immutable recipe");
...
DigitorResult ProductionMediaGraphRuntime::render_frame(
  ...
  if (result != DIGITOR_RESULT_OK) {
    set_diagnostic(diagnostic,
                   decode_diagnostic.empty()
                       ? "production hardware decode failed"
  ...
  if (!decoded.gpu_frame || !decoded.gpu_frame->ready()) {
    set_diagnostic(
        diagnostic,
        "production decoder did not return a ready GPU-resident frame; CPU "
...
DigitorResult ProductionMediaGraphRuntime::preview(
    FrameNumber frame_number,
    ProcessedGpuFramePtr* out_frame,
    std::string* diagnostic) noexcept {
  try {
    if (!presenter_) {
      set_diagnostic(diagnostic,
                     "native Flutter preview presenter is not initialized");
      return DIGITOR_RESULT_NOT_INITIALIZED;
    }
    ...
    if (present_result != DIGITOR_RESULT_OK) {
      set_diagnostic(diagnostic,
                     present_diagnostic.empty()
                         ? "native Flutter GPU presentation failed"
                         : std::move(present_diagnostic));
    ...
    if (out_frame) *out_frame = rendered.frame;
    {
      ...
      // This coordinator has no readback operation by design.
...
DigitorResult ProductionMediaGraphRuntime::export_frames(
  ...
  try {
    ...
    if (!encoder_callbacks_complete(encoder_callbacks_)) {
      set_diagnostic(diagnostic,
                     "production hardware encoder callbacks are incomplete");
      return DIGITOR_RESULT_NOT_INITIALIZED;
    }
    if (config.backend == EncoderBackend::software) {
      set_diagnostic(
          diagnostic,
          "software export is forbidden for the strict production GPU path");
    ...
    // These are invariants of this runtime, not caller-selectable preferences.
    ...
    if (result != DIGITOR_RESULT_OK) {
      clear_active();
      set_diagnostic(diagnostic,
                     encode_diagnostic.empty()
                         ? "failed to start production hardware encoder"
                         : std::move(encode_diagnostic));
      return result;
    }
    ...
    for (const FrameNumber frame_number : frame_numbers) {
      ...
      if (result != DIGITOR_RESULT_OK) {
        ...
        set_diagnostic(diagnostic,
                       encode_diagnostic.empty()
                           ? "production GPU frame submission failed"
                           : std::move(encode_diagnostic));
        return result;
      }
    ...
    if (result != DIGITOR_RESULT_OK) {
      set_diagnostic(diagnostic,
                     encode_diagnostic.empty()
                         ? "production hardware export finalize failed"
                         : std::move(encode_diagnostic));
      return result;
    }
    return DIGITOR_RESULT_OK;
