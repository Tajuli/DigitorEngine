...
public:
  ...
  DigitorResult execute_process_primary_wheels_gpu(const GpuSourceResource&s,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{
    ...
    begin_grade_provenance(DIGITOR_RENDERER_OPENGL_ES,true,i_.device_name,"GLES driver","primary-wheels-glsl-es-v1","GL framebuffer primary GPU source");auto cached=color_program(false,vs,fs);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;GLuint p=cached->program;auto baseline=counts();provenance_.resources_before=baseline;auto owner=std::make_shared<GlPreviewOwner>();owner->program=p;owner->pipeline=cached;owner->context=context;owner->upstream=prior;
  ...
  DigitorResult execute_process_curves_gpu(std::span<const Color> src,uint32_t width,uint32_t height,int64_t timestamp,const CompiledRgbCurves&cc,ProcessedGpuFramePtr&out)noexcept override{
    ...
    auto cached=color_program(true,vs,fs);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;GLuint p=cached->program;auto baseline=counts();provenance_.resources_before=baseline;auto owner=std::shared_ptr<GlPreviewOwner>(new(std::nothrow)GlPreviewOwner{});if(!owner)return DIGITOR_RESULT_OUT_OF_MEMORY;owner->program=p;owner->pipeline=cached;owner->context=context;
