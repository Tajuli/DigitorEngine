...
bool create_metal_native_node_pipeline(const NativeNodePlatformFactoryContext& ctx,
 ...
 out={}; @autoreleasepool {
  ...
  id<MTLDevice> device=(__bridge id<MTLDevice>)reinterpret_cast<void*>(static_cast<std::uintptr_t>(ctx.device));
  ...
  out.pipeline=static_cast<NativeNodeNativeHandle>(reinterpret_cast<std::uintptr_t>((__bridge_retained void*)pipeline));
...
void destroy_metal_native_node_pipeline(const NativeNodePlatformFactoryContext&,const NativeNodeBackendPipelineHandle& h) noexcept {
 if(h.pipeline) CFRelease(reinterpret_cast<CFTypeRef>(static_cast<std::uintptr_t>(h.pipeline)));
}
bool record_metal_native_node_dispatch(const NativeNodePlatformFactoryContext& ctx,const NativeNodeBackendPipelineHandle& h,
 ...
 @autoreleasepool {
  ...
  id<MTLComputeCommandEncoder> encoder=(__bridge id<MTLComputeCommandEncoder>)reinterpret_cast<void*>(static_cast<std::uintptr_t>(ctx.command_context));
  id<MTLComputePipelineState> pipeline=(__bridge id<MTLComputePipelineState>)reinterpret_cast<void*>(static_cast<std::uintptr_t>(h.pipeline));
  [encoder setComputePipelineState:pipeline];
  for(const auto& t:resources.textures){ if(!t.native_texture){diagnostic="invalid Metal texture";return false;} id<MTLTexture> texture=(__bridge id<MTLTexture>)reinterpret_cast<void*>(static_cast<std::uintptr_t>(t.native_texture)); [encoder setTexture:texture atIndex:t.slot]; }
