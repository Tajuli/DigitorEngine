...
precision highp float;precision highp int;layout(local_size_x=8,local_size_y=8)in;layout(binding=0,rgba32f)uniform readonly highp image2D src;layout(binding=1,r32f)uniform writeonly highp image2D outm;layout(std140,binding=2)uniform P{vec4 hr;vec4 sr;vec4 lr;float cb;float cw;float denoise;float blur;uint inv;uint width;uint height;uint pad;}p;
