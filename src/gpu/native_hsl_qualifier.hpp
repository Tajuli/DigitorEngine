...
struct NativeHslQualifierParameters {
  std::array<float,4> hue{}, saturation{}, luminance{}, cleanup{};
  std::uint32_t width{}, height{}, pixel_count{}, flags{};
};
...
[[nodiscard]] NativeHslQualifierParameters native_hsl_qualifier_parameters(
    const HslQualifierParameters&, std::uint32_t width, std::uint32_t height);
