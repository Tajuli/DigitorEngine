...
NativeNodeNativeHandle encode_vulkan_handle(Handle handle) noexcept {
...
template <typename Handle>
Handle decode_vulkan_handle(NativeNodeNativeHandle value) noexcept {
  if constexpr (std::is_pointer_v<Handle>) {
...
bool create_vulkan_native_node_pipeline(
  ...
  const auto device = decode_vulkan_handle<VkDevice>(ctx.device);
  const auto contract =
      native_node_pipeline_contract(compiled.backend, compiled.kernel);
  ...
  if (vkCreateDescriptorSetLayout(device, &dli, nullptr, &descriptor_layout) !=
      VK_SUCCESS) {
  ...
  if (vkCreatePipelineLayout(device, &pli, nullptr, &pipeline_layout) !=
      VK_SUCCESS) {
  ...
  out.pipeline = encode_vulkan_handle(pipeline);
  out.layout = encode_vulkan_handle(pipeline_layout);
  out.auxiliary = encode_vulkan_handle(descriptor_layout);
  diagnostic.clear();
  return true;
#else
  (void)ctx;
...
void destroy_vulkan_native_node_pipeline(
  ...
  const auto device = decode_vulkan_handle<VkDevice>(ctx.device);
  if (handle.pipeline) {
    vkDestroyPipeline(device, decode_vulkan_handle<VkPipeline>(handle.pipeline),
                      nullptr);
  }
  if (handle.layout) {
    vkDestroyPipelineLayout(
        device, decode_vulkan_handle<VkPipelineLayout>(handle.layout), nullptr);
  }
  if (handle.auxiliary) {
    vkDestroyDescriptorSetLayout(
        device,
        decode_vulkan_handle<VkDescriptorSetLayout>(handle.auxiliary), nullptr);
  }
#else
  (void)ctx;
  (void)handle;
...
bool create_d3d12_native_node_pipeline(
  ...
  auto* device = reinterpret_cast<ID3D12Device*>(
      static_cast<std::uintptr_t>(ctx.device));
  const auto contract =
      native_node_pipeline_contract(compiled.backend, compiled.kernel);
  ...
  if (FAILED(D3D12SerializeRootSignature(
          &rsd, D3D_ROOT_SIGNATURE_VERSION_1, &serialized, &errors))) {
  ...
  out.pipeline = static_cast<NativeNodeNativeHandle>(
      reinterpret_cast<std::uintptr_t>(pso));
  out.layout = static_cast<NativeNodeNativeHandle>(
...
void destroy_d3d12_native_node_pipeline(
  ...
  if (handle.pipeline) {
    reinterpret_cast<ID3D12PipelineState*>(
        static_cast<std::uintptr_t>(handle.pipeline))
        ->Release();
  }
  if (handle.layout) {
    reinterpret_cast<ID3D12RootSignature*>(
        static_cast<std::uintptr_t>(handle.layout))
        ->Release();
  }
#else
  (void)handle;
...
bool record_vulkan_native_node_dispatch(
  ...
  if (!ctx.device || !ctx.command_context || !ctx.descriptor_context ||
      ...
      geometry.groups_x == 0 || geometry.groups_y == 0 ||
      geometry.groups_z == 0) {
  ...
  const auto command_buffer =
      decode_vulkan_handle<VkCommandBuffer>(ctx.command_context);
  const auto descriptor_pool =
      decode_vulkan_handle<VkDescriptorPool>(ctx.descriptor_context);
  const auto descriptor_layout =
      decode_vulkan_handle<VkDescriptorSetLayout>(handle.auxiliary);
  ...
  vkUpdateDescriptorSets(
      device, static_cast<std::uint32_t>(resources.textures.size()), writes, 0,
      nullptr);
  vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                    decode_vulkan_handle<VkPipeline>(handle.pipeline));
  vkCmdBindDescriptorSets(
      command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
      decode_vulkan_handle<VkPipelineLayout>(handle.layout), 0, 1,
      &descriptor_set, 0, nullptr);
  if (!resources.constants.empty()) {
    vkCmdPushConstants(
        command_buffer, decode_vulkan_handle<VkPipelineLayout>(handle.layout),
...
bool record_d3d12_native_node_dispatch(
  ...
  auto* command_list = reinterpret_cast<ID3D12GraphicsCommandList*>(
      static_cast<std::uintptr_t>(ctx.command_context));
  auto* heap = reinterpret_cast<ID3D12DescriptorHeap*>(
      static_cast<std::uintptr_t>(ctx.descriptor_context));
...
void NativeNodeDescriptorRetirementQueue::retain(
    NativeNodeNativeHandle native_handle, std::uint64_t completion_value) {
  if (!native_handle) return;
  pending_.push_back({native_handle, completion_value});
}

std::vector<NativeNodeNativeHandle> NativeNodeDescriptorRetirementQueue::collect(
    std::uint64_t completed_value) {
  std::vector<NativeNodeNativeHandle> ready;
...
bool record_vulkan_native_node_barriers(
  ...
  for (const auto& t : transitions) {
    ...
    b.image = decode_vulkan_handle<VkImage>(t.native_resource);
...
bool record_d3d12_native_node_barriers(
  ...
  for (const auto& t : transitions) {
    ...
    b.Transition.pResource = reinterpret_cast<ID3D12Resource*>(
        static_cast<std::uintptr_t>(t.native_resource));
