...
void validate_range(const QualifierRange& value, const char* name) {
  if (!std::isfinite(value.low) || !std::isfinite(value.high) ||
      !std::isfinite(value.softness) || value.low < 0.0f ||
      value.low > 1.0f || value.high < 0.0f || value.high > 1.0f ||
      value.softness < 0.0f || value.softness > 1.0f) {
...
void validate(const QualifierSettings& settings) {
  ...
  if (!std::isfinite(settings.blur) || !std::isfinite(settings.denoise) ||
      !std::isfinite(settings.clean_black) ||
      !std::isfinite(settings.clean_white) || settings.blur < 0.0f ||
...
float hue_weight(float hue, const QualifierRange& range) noexcept {
  ...
  const QualifierRange upper{range.low, 1.0f, range.softness};
  const QualifierRange lower{0.0f, range.high, range.softness};
  return std::max(linear_weight(hue, upper), linear_weight(hue, lower));
}

void rgb_to_hsl(Color color, float& hue, float& saturation,
...
float base_matte(Color color, const QualifierSettings& settings) noexcept {
  if (!std::isfinite(color.r) || !std::isfinite(color.g) ||
      !std::isfinite(color.b)) return 0.0f;
  float hue{}, saturation{}, luminance{};
  rgb_to_hsl(color, hue, saturation, luminance);
  float value = hue_weight(hue, settings.hue) *
                linear_weight(saturation, settings.saturation) *
                linear_weight(luminance, settings.luminance);
...
bool HslQualifierParameters::is_identity() const noexcept {
  ...
  return p.hue.low == 0.0f && p.hue.high == 1.0f &&
         p.hue.softness == 0.0f && p.saturation.low == 0.0f &&
         p.saturation.high == 1.0f && p.saturation.softness == 0.0f &&
         p.luminance.low == 0.0f && p.luminance.high == 1.0f &&
         p.luminance.softness == 0.0f && p.blur == 0.0f &&
         p.denoise == 0.0f && p.clean_black == 0.0f &&
...
float apply_hsl_qualifier_reference(
    Color color, const HslQualifierParameters& parameters) noexcept {
