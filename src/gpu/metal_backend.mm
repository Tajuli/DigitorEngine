...
public:
  ...
  std::shared_ptr<MetalPipelineBundle> color_pipeline(bool curves,
                                                      NSString *code) noexcept {
    std::string identity =
        curves ? "rgb-curves:metal-v1" : "primary-wheels:metal-v1";
          ...
          @autoreleasepool {
            @try {
              ...
              bundle->function = [bundle->library
                  newFunctionWithName:curves ? @"curves" : @"wheels"];
              if (bundle->function)
                ++metal_live.pipelines;
  ...
  DigitorResult execute_process_primary_wheels_gpu(
    ...
    @autoreleasepool {
      @try {
        ...
        if (!end_encoder(e, "compute encoder endEncoding") ||
  ...
  DigitorResult execute_validation_readback_primary_wheels(
    ...
    @try {
      ...
      if (!end_encoder(blit, "validation blit endEncoding") ||
          !commit_and_wait(command))
        return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
  ...
  DigitorResult
  execute_process_curves_gpu(std::span<const Color> source, std::uint32_t width,
      ...
      @try {
        static NSString *code =
            @"#include <metal_stdlib>\nusing namespace metal;"
             ...
             ";"
             "}";
        auto cached = color_pipeline(true, code);
        id<MTLComputePipelineState> pipe = cached ? cached->pipeline : nil;
            ...
            [this](const ProcessedGpuFramePtr &f,
              ...
              @try {
                @autoreleasepool {
                  ...
                  if (!end_encoder(blit, "consumer blit endEncoding") ||
                      !commit_and_wait(command))
                    return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
                  return DIGITOR_RESULT_OK;
                }
  ...
  DigitorResult execute_present_gpu_frame(
    ...
    @try {
      ...
      [b copyFromTexture:owner->output
           ...
           destinationLevel:0
          destinationOrigin:MTLOriginMake(0, 0, 0)];
      auto r = end_encoder(b, "direct preview blit endEncoding") &&
                       commit_and_wait(c)
                   ? DIGITOR_RESULT_OK
