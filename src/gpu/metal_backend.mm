...
struct MetalConsumerOwner { id<MTLDevice> device;id<MTLTexture> texture;id<MTLCommandQueue> queue; };
struct MetalPipelineBundle { id<MTLLibrary> library;id<MTLFunction> function;id<MTLComputePipelineState> pipeline; };
...
public:
    ...
    std::shared_ptr<MetalPipelineBundle> color_pipeline(bool curves,NSString*code) noexcept {
      std::string identity=curves?"rgb-curves:metal-v1":"primary-wheels:metal-v1";if(gpu_failure_point()==GpuFailurePoint::PipelineCreation)identity += ":injected-create";NativePipelineCacheKey key{DIGITOR_RENDERER_METAL,reinterpret_cast<std::uintptr_t>((__bridge void*)device_),identity,1,GpuPrecisionMode::Float32,DIGITOR_PIXEL_FORMAT_RGBA32_FLOAT};
      return std::static_pointer_cast<MetalPipelineBundle>(pipeline_cache_.get_or_create(key,[&]()->NativePipelineCache::Object{@autoreleasepool{if(gpu_failure_point()==GpuFailurePoint::ShaderCompilation){(void)injected_failure(GpuFailurePoint::ShaderCompilation);return {};}NSError*error=nil;auto bundle=std::make_shared<MetalPipelineBundle>();bundle->library=[device_ newLibraryWithSource:code options:nil error:&error];bundle->function=[bundle->library newFunctionWithName:curves?@"curves":@"wheels"];if(gpu_failure_point()==GpuFailurePoint::PipelineCreation){(void)injected_failure(GpuFailurePoint::PipelineCreation);return {};}bundle->pipeline=bundle->function?[device_ newComputePipelineStateWithFunction:bundle->function error:&error]:nil;if(!bundle->library||!bundle->function||!bundle->pipeline)return {};return std::static_pointer_cast<void>(bundle);}}));
    }

    bool initialize(bool) override { return device_ != nil; }
    void shutdown() noexcept override { pipeline_cache_.invalidate_device(DIGITOR_RENDERER_METAL,reinterpret_cast<std::uintptr_t>((__bridge void*)device_)); }
    ...
    DigitorResult execute_process_primary_wheels_gpu(std::span<const Color> source,
      ...
      @autoreleasepool {
        static NSString*code=@"#include <metal_stdlib>\nusing namespace metal;struct P{float4 lift,gamma,gain,offset;uint4 enabled;uint count,w,h,pad;};float sp(float x,float e){return !isfinite(x)?x:(x<0?-pow(-x,e):pow(x,e));}kernel void wheels(texture2d<float,access::read>i[[texture(0)]],texture2d<float,access::write>o[[texture(1)]],constant P&p[[buffer(0)]],uint2 q[[thread_position_in_grid]]){if(q.x>=p.w||q.y>=p.h)return;float4 c=i.read(q);float a=c.a;if(p.enabled.x)c.rgb+=p.lift.rgb+p.lift.a;if(p.enabled.y)c.rgb=float3(sp(c.r,1/(p.gamma.r*p.gamma.a)),sp(c.g,1/(p.gamma.g*p.gamma.a)),sp(c.b,1/(p.gamma.b*p.gamma.a)));if(p.enabled.z)c.rgb*=p.gain.rgb*p.gain.a;if(p.enabled.w)c.rgb+=p.offset.rgb+p.offset.a;c.a=a;o.write(c,q);}";
        auto cached=color_pipeline(false,code); id<MTLComputePipelineState> pipe=cached?cached->pipeline:nil;
        ...
        [input replaceRegion:MTLRegionMake2D(0,0,width,height) mipmapLevel:0 withBytes:source.data() bytesPerRow:width*sizeof(Color)];struct P{float lift[4],gamma[4],gain[4],offset[4];uint32_t enabled[4],count,w,h,pad;}p{};const auto&x=parameters.values();p={{x.lift.r,x.lift.g,x.lift.b,x.lift_master},{x.gamma.r,x.gamma.g,x.gamma.b,x.gamma_master},{x.gain.r,x.gain.g,x.gain.b,x.gain_master},{x.offset.r,x.offset.g,x.offset.b,x.offset_master},{x.lift_enabled?1u:0u,x.gamma_enabled?1u:0u,x.gain_enabled?1u:0u,x.offset_enabled?1u:0u},uint32_t(source.size()),width,height,0};static_assert(sizeof(P)==96);id<MTLBuffer>pb=[device_ newBufferWithBytes:&p length:sizeof(p) options:MTLResourceStorageModeShared];if(!pb)return DIGITOR_RESULT_OUT_OF_MEMORY;
    ...
    DigitorResult execute_process_primary_wheels_gpu(const GpuSourceResource& source,
      ...
      @autoreleasepool {static NSString*code=@"#include <metal_stdlib>\nusing namespace metal;struct P{float4 lift,gamma,gain,offset;uint4 enabled;uint count,w,h,pad;};float sp(float x,float e){return !isfinite(x)?x:(x<0?-pow(-x,e):pow(x,e));}kernel void wheels(texture2d<float,access::read>i[[texture(0)]],texture2d<float,access::write>o[[texture(1)]],constant P&p[[buffer(0)]],uint2 q[[thread_position_in_grid]]){if(q.x>=p.w||q.y>=p.h)return;float4 c=i.read(q);float a=c.a;if(p.enabled.x)c.rgb+=p.lift.rgb+p.lift.a;if(p.enabled.y)c.rgb=float3(sp(c.r,1/(p.gamma.r*p.gamma.a)),sp(c.g,1/(p.gamma.g*p.gamma.a)),sp(c.b,1/(p.gamma.b*p.gamma.a)));if(p.enabled.z)c.rgb*=p.gain.rgb*p.gain.a;if(p.enabled.w)c.rgb+=p.offset.rgb+p.offset.a;c.a=a;o.write(c,q);}";auto cached=color_pipeline(false,code); id<MTLComputePipelineState> pipe=cached?cached->pipeline:nil;MTLTextureDescriptor*d=[MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA32Float width:source.width height:source.height mipmapped:NO];d.storageMode=MTLStorageModePrivate;d.usage=MTLTextureUsageShaderRead|MTLTextureUsageShaderWrite;id<MTLTexture>output=[device_ newTextureWithDescriptor:d];id<MTLTexture>preview=[device_ newTextureWithDescriptor:d];id<MTLCommandQueue>queue=[device_ newCommandQueue];if(!pipe||!output||!preview||!queue)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;struct P{float lift[4],gamma[4],gain[4],offset[4];uint32_t enabled[4],count,w,h,pad;}p{};const auto&x=parameters.values();p={{x.lift.r,x.lift.g,x.lift.b,x.lift_master},{x.gamma.r,x.gamma.g,x.gamma.b,x.gamma_master},{x.gain.r,x.gain.g,x.gain.b,x.gain_master},{x.offset.r,x.offset.g,x.offset.b,x.offset_master},{x.lift_enabled?1u:0u,x.gamma_enabled?1u:0u,x.gain_enabled?1u:0u,x.offset_enabled?1u:0u},source.width*source.height,source.width,source.height,0};id<MTLBuffer>pb=[device_ newBufferWithBytes:&p length:sizeof(p) options:MTLResourceStorageModeShared];id<MTLCommandBuffer>command=[queue commandBuffer];id<MTLComputeCommandEncoder>e=[command computeCommandEncoder];[e setComputePipelineState:pipe];[e setTexture:prior->output atIndex:0];[e setTexture:output atIndex:1];[e setBuffer:pb offset:0 atIndex:0];NSUInteger n=std::min<NSUInteger>(pipe.maxTotalThreadsPerThreadgroup,64);[e dispatchThreads:MTLSizeMake(source.width,source.height,1) threadsPerThreadgroup:MTLSizeMake(n,1,1)];[e endEncoding];[command commit];[command waitUntilCompleted];if(command.status==MTLCommandBufferStatusError)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto owner=std::shared_ptr<void>(new MetalPreviewOwner{output,preview,queue,prior},[](void*v){delete static_cast<MetalPreviewOwner*>(v);});static std::atomic_uint64_t ids{200000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_METAL,GpuFrameMetadata{source.width,source.height,source.format,GpuFrameAlpha::straight,timestamp,source.color_metadata_identity},ids++,owner,std::make_shared<std::atomic_bool>(true),true);provenance_.primary_wheels_source_bound=provenance_.primary_wheels_destination_bound=provenance_.primary_wheels_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;return DIGITOR_RESULT_OK;}
    }
    ...
    DigitorResult execute_process_curves_gpu(std::span<const Color> source,
      ...
      @autoreleasepool {
        ...
        auto cached=color_pipeline(true,code); id<MTLComputePipelineState> pipe=cached?cached->pipeline:nil;
    ...
    DigitorResult execute_process_curves_gpu(const GpuSourceResource& source,std::int64_t timestamp,const CompiledRgbCurves& compiled,ProcessedGpuFramePtr& out) noexcept override {
      out.reset();auto prior=std::static_pointer_cast<MetalPreviewOwner>(native_owner(*source.frame));if(!prior||prior->output==nil)return DIGITOR_RESULT_INVALID_ARGUMENT;
      @autoreleasepool {static NSString*code=@"#include <metal_stdlib>\nusing namespace metal;struct M{float lo,hi,first,last,sb,sa;uint extrap,enabled;};struct P{M m[4];uint size,w,h;};float cv(device const float*l,constant P&p,uint k,float x){M m=p.m[k];if(!m.enabled||!isfinite(x))return x;if(x<m.lo)return m.extrap==2?m.first+m.sb*(x-m.lo):m.first;if(x>m.hi)return m.extrap==2?m.last+m.sa*(x-m.hi):m.last;float u=(x-m.lo)/(m.hi-m.lo)*float(p.size-1);uint a=min(uint(u),p.size-1),b=min(a+1,p.size-1);return mix(l[k*p.size+a],l[k*p.size+b],u-float(a));}kernel void curves(texture2d<float,access::read>i[[texture(0)]],texture2d<float,access::write>o[[texture(1)]],device const float*l[[buffer(0)]],constant P&p[[buffer(1)]],uint2 q[[thread_position_in_grid]]){if(q.x>=p.w||q.y>=p.h)return;float4 c=i.read(q);float a=c.a;c.r=cv(l,p,0,c.r);c.g=cv(l,p,0,c.g);c.b=cv(l,p,0,c.b);c.r=cv(l,p,1,c.r);c.g=cv(l,p,2,c.g);c.b=cv(l,p,3,c.b);c.a=a;o.write(c,q);}";auto cached=color_pipeline(true,code); id<MTLComputePipelineState> pipe=cached?cached->pipeline:nil;MTLTextureDescriptor*d=[MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA32Float width:source.width height:source.height mipmapped:NO];d.storageMode=MTLStorageModePrivate;d.usage=MTLTextureUsageShaderRead|MTLTextureUsageShaderWrite;id<MTLTexture>output=[device_ newTextureWithDescriptor:d];id<MTLTexture>preview=[device_ newTextureWithDescriptor:d];id<MTLCommandQueue>queue=[device_ newCommandQueue];if(!pipe||!output||!preview||!queue)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;struct M{float lo,hi,first,last,sb,sa;uint32_t extrap,enabled;};struct P{M m[4];uint32_t size,w,h;}p{};std::vector<float>lut;for(unsigned k=0;k<4;k++){const auto&c=compiled.curves()[k];p.m[k]={c.domain_min,c.domain_max,c.first_value,c.last_value,c.slope_before,c.slope_after,uint32_t(c.extrapolation),c.enabled&&!c.identity?1u:0u};lut.insert(lut.end(),c.samples.begin(),c.samples.end());}p.size=compiled.lut_size();p.w=source.width;p.h=source.height;id<MTLBuffer>lb=[device_ newBufferWithBytes:lut.data() length:lut.size()*sizeof(float) options:MTLResourceStorageModeShared];id<MTLCommandBuffer>command=[queue commandBuffer];id<MTLComputeCommandEncoder>e=[command computeCommandEncoder];[e setComputePipelineState:pipe];[e setTexture:prior->output atIndex:0];[e setTexture:output atIndex:1];[e setBuffer:lb offset:0 atIndex:0];[e setBytes:&p length:sizeof(p) atIndex:1];NSUInteger n=std::min<NSUInteger>(pipe.maxTotalThreadsPerThreadgroup,64);[e dispatchThreads:MTLSizeMake(source.width,source.height,1) threadsPerThreadgroup:MTLSizeMake(n,1,1)];[e endEncoding];[command commit];[command waitUntilCompleted];if(command.status==MTLCommandBufferStatusError)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto owner=std::shared_ptr<void>(new MetalPreviewOwner{output,preview,queue,prior},[](void*v){delete static_cast<MetalPreviewOwner*>(v);});static std::atomic_uint64_t ids{300000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_METAL,GpuFrameMetadata{source.width,source.height,source.format,GpuFrameAlpha::straight,timestamp,source.color_metadata_identity},ids++,owner,std::make_shared<std::atomic_bool>(true),true);provenance_.curve_source_bound=provenance_.curve_destination_bound=provenance_.curve_lut_bound=provenance_.curve_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;provenance_.readback_performed=false;return DIGITOR_RESULT_OK;}
    }
    DigitorResult execute_create_preview_consumer(const ProcessedGpuFramePtr&frame,std::shared_ptr<PreviewConsumerDestination>&out)noexcept override {
      ...
      @autoreleasepool {out.reset();if(!frame||!device_)return DIGITOR_RESULT_NOT_INITIALIZED;const auto&m=frame->metadata();MTLTextureDescriptor*d=[MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA32Float width:m.width height:m.height mipmapped:NO];d.storageMode=MTLStorageModePrivate;d.usage=MTLTextureUsageShaderRead;id<MTLTexture>texture=[device_ newTextureWithDescriptor:d];id<MTLCommandQueue>queue=[device_ newCommandQueue];if(!texture||!queue)return DIGITOR_RESULT_OUT_OF_MEMORY;auto owner=std::shared_ptr<void>(new MetalConsumerOwner{device_,texture,queue},[](void*v){delete static_cast<MetalConsumerOwner*>(v);});static std::atomic_uint64_t tokens{1};out=std::make_shared<PreviewConsumerDestination>(PreviewConsumerMetadata{DIGITOR_RENDERER_METAL,this,m.width,m.height,m.format,GpuPrecisionMode::Float32},tokens++,owner,std::make_shared<std::atomic_bool>(true),[this](const ProcessedGpuFramePtr&f,const std::shared_ptr<void>&d){if(gpu_failure_point()==GpuFailurePoint::PreviewPresentation)return injected_failure(GpuFailurePoint::PreviewPresentation);@autoreleasepool {auto source=std::static_pointer_cast<MetalPreviewOwner>(native_owner(*f));auto destination=std::static_pointer_cast<MetalConsumerOwner>(d);if(!source||!destination||destination->device!=device_)return DIGITOR_RESULT_INVALID_ARGUMENT;id<MTLCommandBuffer>command=[destination->queue commandBuffer];id<MTLBlitCommandEncoder>blit=[command blitCommandEncoder];const auto&m=f->metadata();[blit copyFromTexture:source->output sourceSlice:0 sourceLevel:0 sourceOrigin:MTLOriginMake(0,0,0) sourceSize:MTLSizeMake(m.width,m.height,1) toTexture:destination->texture destinationSlice:0 destinationLevel:0 destinationOrigin:MTLOriginMake(0,0,0)];[blit endEncoding];[command commit];[command waitUntilCompleted];return command.status==MTLCommandBufferStatusCompleted?DIGITOR_RESULT_OK:DIGITOR_RESULT_BACKEND_UNAVAILABLE;}});return DIGITOR_RESULT_OK;}
    }
