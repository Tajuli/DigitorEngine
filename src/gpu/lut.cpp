...
Color Lut1D::sample(Color c,LutInterpolation interpolation)const{Color o=c;for(int k=0;k<3;++k){const float domain=channel(domain_max_,k)-channel(domain_min_,k);float x=std::clamp((channel(c,k)-channel(domain_min_,k))/domain,0.f,1.f)*(values_.size()-1);std::size_t lo=static_cast<std::size_t>(x),hi=std::min(lo+1,values_.size()-1);float t=interpolation==LutInterpolation::nearest?(x-lo>=.5f?1.f:0.f):x-lo;float v=mix(channel(values_[lo],k),channel(values_[hi],k),t);if(k==0)o.r=v;else if(k==1)o.g=v;else o.b=v;}return o;}
