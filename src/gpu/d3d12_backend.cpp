...
HRESULT compile_log_wheels(ComPtr<ID3DBlob> &bytecode,
                           ComPtr<ID3DBlob> &errors) noexcept {
  const D3D_SHADER_MACRO macros[]{{"DIGITOR_TEXTURE_OUTPUT", "1"},
                                  {nullptr, nullptr}};
  return D3DCompile(digitor_log_wheels_hlsl, sizeof(digitor_log_wheels_hlsl),
                    "log_wheels.hlsl", macros, nullptr, "main", "cs_5_1",
                    D3DCOMPILE_OPTIMIZATION_LEVEL3, 0,
                    bytecode.ReleaseAndGetAddressOf(),
                    errors.ReleaseAndGetAddressOf());
}

HRESULT compile_hsl_qualifier(ComPtr<ID3DBlob> &bytecode, ComPtr<ID3DBlob> &errors) noexcept {
  const D3D_SHADER_MACRO macros[]{{"DIGITOR_TEXTURE_OUTPUT", "1"},{nullptr,nullptr}};
  return D3DCompile(digitor_hsl_qualifier_hlsl,sizeof(digitor_hsl_qualifier_hlsl),"hsl_qualifier.hlsl",macros,nullptr,"main","cs_5_1",D3DCOMPILE_OPTIMIZATION_LEVEL3,0,bytecode.ReleaseAndGetAddressOf(),errors.ReleaseAndGetAddressOf());
}
...
public:
  ...
  std::shared_ptr<D3DPipelineBundle> color_pipeline(int operation) noexcept {
    ...
    std::string identity =
        curves ? "rgb-curves:cs5.1:root-v1" : log_wheels ? "log-wheels:cs5.1:root-v1" : qualifier ? "hsl-qualifier:cs5.1:root-v1" : "primary-wheels:cs5.1:root-v1";
    ...
    return std::static_pointer_cast<
        ...
        key, [&]() -> NativePipelineCache::Object {
          ...
          if (FAILED(curves ? compile_rgb_curves(true, bundle->shader, error)
                            : log_wheels ? compile_log_wheels(bundle->shader, error)
                            : qualifier ? compile_hsl_qualifier(bundle->shader, error)
          ...
          roots[2].Constants = {
              ...
              curves ? UINT(sizeof(NativeRgbCurvesParameters) / 4)
                     : log_wheels ? UINT(sizeof(NativeLogWheelsParameters) / 4)
                     : qualifier ? UINT(sizeof(NativeHslQualifierParameters) / 4)
  ...
  DigitorResult execute_process_log_wheels_gpu(
    ...
    if (!width || !height || source.size() != size_t(width) * height)
      return DIGITOR_RESULT_INVALID_ARGUMENT;
    auto cached = color_pipeline(2);
    if (!cached)
      return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto root = cached->root;
    auto pipeline = cached->pipeline;
    constexpr UINT dwords = sizeof(NativeLogWheelsParameters) / 4;
    auto committed = [&](GpuFailurePoint stage, const char *operation,
                         const D3D12_RESOURCE_DESC &d, D3D12_HEAP_TYPE ht,
                         D3D12_RESOURCE_STATES st, ComPtr<ID3D12Resource> &r) {
      D3D12_HEAP_PROPERTIES hp{};
    ...
    provenance_.log_wheels_shader_identity = "log_wheels.hlsl:DXIL-v1";
    provenance_.log_wheels_pipeline_identity =
        "ID3D12PipelineState:log-wheels-v1";
  ...
  DigitorResult execute_process_hsl_qualifier_gpu(
    ...
    if (!width || !height || source.size() != size_t(width) * height)
      return DIGITOR_RESULT_INVALID_ARGUMENT;
    auto cached = color_pipeline(3);
    if (!cached)
      return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto root = cached->root;
    auto pipeline = cached->pipeline;
    constexpr UINT dwords = sizeof(NativeHslQualifierParameters) / 4;
    auto committed = [&](GpuFailurePoint stage, const char *operation,
                         const D3D12_RESOURCE_DESC &d, D3D12_HEAP_TYPE ht,
                         D3D12_RESOURCE_STATES st, ComPtr<ID3D12Resource> &r) {
      D3D12_HEAP_PROPERTIES hp{};
    ...
    provenance_.hsl_qualifier_shader_identity = "hsl_qualifier.hlsl:DXIL-v1";
    provenance_.hsl_qualifier_pipeline_identity =
        "ID3D12PipelineState:hsl-qualifier-v1";
