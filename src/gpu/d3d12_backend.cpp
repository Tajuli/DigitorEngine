...
public:
  ...
  DigitorResult execute_process_primary_wheels_gpu(std::span<const Color>source,uint32_t width,uint32_t height,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();begin_grade_provenance(DIGITOR_RENDERER_D3D12,true,info_.device_name,"D3DCompile cs_5_1","primary_wheels.hlsl:texture","ID3D12PipelineState:primary-wheels-v1");if(!width||!height||source.size()!=size_t(width)*height)return DIGITOR_RESULT_INVALID_ARGUMENT;ComPtr<ID3DBlob>shader,error;if(FAILED(compile_primary_wheels(shader,error)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    D3D12_DESCRIPTOR_RANGE ranges[2]{};ranges[0]={D3D12_DESCRIPTOR_RANGE_TYPE_SRV,1,0};ranges[1]={D3D12_DESCRIPTOR_RANGE_TYPE_UAV,1,0};D3D12_ROOT_PARAMETER roots[3]{};for(int n=0;n<2;n++){roots[n].ParameterType=D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;roots[n].DescriptorTable={1,&ranges[n]};}roots[2].ParameterType=D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;constexpr UINT dwords=sizeof(NativePrimaryWheelsParameters)/4;roots[2].Constants={0,0,dwords};D3D12_ROOT_SIGNATURE_DESC rd{3,roots,0,nullptr,D3D12_ROOT_SIGNATURE_FLAG_NONE};ComPtr<ID3DBlob>sig,re;if(FAILED(D3D12SerializeRootSignature(&rd,D3D_ROOT_SIGNATURE_VERSION_1,&sig,&re)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ComPtr<ID3D12RootSignature>root;if(FAILED(device_->CreateRootSignature(0,sig->GetBufferPointer(),sig->GetBufferSize(),IID_PPV_ARGS(&root))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;D3D12_COMPUTE_PIPELINE_STATE_DESC pd{};pd.pRootSignature=root.Get();pd.CS={shader->GetBufferPointer(),shader->GetBufferSize()};ComPtr<ID3D12PipelineState>pipeline;if(FAILED(device_->CreateComputePipelineState(&pd,IID_PPV_ARGS(&pipeline))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
  ...
  DigitorResult execute_process_primary_wheels_gpu(const GpuSourceResource&s,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();auto prior=std::static_pointer_cast<D3DPreviewOwner>(native_owner(*s.frame));if(!prior||!prior->output||prior->output_state!=D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)return DIGITOR_RESULT_INVALID_ARGUMENT;ComPtr<ID3DBlob>shader,error;if(FAILED(compile_primary_wheels(shader,error)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    D3D12_DESCRIPTOR_RANGE ranges[2]{};ranges[0]={D3D12_DESCRIPTOR_RANGE_TYPE_SRV,1,0};ranges[1]={D3D12_DESCRIPTOR_RANGE_TYPE_UAV,1,0};D3D12_ROOT_PARAMETER roots[3]{};for(int n=0;n<2;n++){roots[n].ParameterType=D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;roots[n].DescriptorTable={1,&ranges[n]};}roots[2].ParameterType=D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;constexpr UINT dwords=sizeof(NativePrimaryWheelsParameters)/4;roots[2].Constants={0,0,dwords};D3D12_ROOT_SIGNATURE_DESC rd{3,roots};ComPtr<ID3DBlob>sig,re;if(FAILED(D3D12SerializeRootSignature(&rd,D3D_ROOT_SIGNATURE_VERSION_1,&sig,&re)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ComPtr<ID3D12RootSignature>root;if(FAILED(device_->CreateRootSignature(0,sig->GetBufferPointer(),sig->GetBufferSize(),IID_PPV_ARGS(&root))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;D3D12_COMPUTE_PIPELINE_STATE_DESC pd{};pd.pRootSignature=root.Get();pd.CS={shader->GetBufferPointer(),shader->GetBufferSize()};ComPtr<ID3D12PipelineState>pipeline;if(FAILED(device_->CreateComputePipelineState(&pd,IID_PPV_ARGS(&pipeline))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
  ...
  DigitorResult execute_process_curves_gpu(const GpuSourceResource&s,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();auto prior=std::static_pointer_cast<D3DPreviewOwner>(native_owner(*s.frame));if(!prior||!prior->output||prior->output_state!=D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)return DIGITOR_RESULT_INVALID_ARGUMENT;ComPtr<ID3DBlob>shader,error;if(FAILED(compile_rgb_curves(true,shader,error)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;D3D12_RESOURCE_DESC td=prior->output->GetDesc();if(td.Dimension!=D3D12_RESOURCE_DIMENSION_TEXTURE2D||td.Width!=s.width||td.Height!=s.height||td.Format!=DXGI_FORMAT_R32G32B32A32_FLOAT)return DIGITOR_RESULT_INVALID_ARGUMENT;
    D3D12_DESCRIPTOR_RANGE ranges[2]{};ranges[0]={D3D12_DESCRIPTOR_RANGE_TYPE_SRV,2,0};ranges[1]={D3D12_DESCRIPTOR_RANGE_TYPE_UAV,1,0};D3D12_ROOT_PARAMETER roots[3]{};roots[0].ParameterType=D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;roots[0].DescriptorTable={1,&ranges[0]};roots[1].ParameterType=D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;roots[1].DescriptorTable={1,&ranges[1]};roots[2].ParameterType=D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;constexpr UINT dwords=sizeof(NativeRgbCurvesParameters)/4;roots[2].Constants={0,0,dwords};D3D12_ROOT_SIGNATURE_DESC rd{3,roots};ComPtr<ID3DBlob>sig,re;if(FAILED(D3D12SerializeRootSignature(&rd,D3D_ROOT_SIGNATURE_VERSION_1,&sig,&re)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ComPtr<ID3D12RootSignature>root;if(FAILED(device_->CreateRootSignature(0,sig->GetBufferPointer(),sig->GetBufferSize(),IID_PPV_ARGS(&root))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;D3D12_COMPUTE_PIPELINE_STATE_DESC pd{};pd.pRootSignature=root.Get();pd.CS={shader->GetBufferPointer(),shader->GetBufferSize()};ComPtr<ID3D12PipelineState>pipeline;if(FAILED(device_->CreateComputePipelineState(&pd,IID_PPV_ARGS(&pipeline))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
