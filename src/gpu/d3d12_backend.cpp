...
HRESULT compile_primary_wheels(ComPtr<ID3DBlob>&bytecode,ComPtr<ID3DBlob>&errors)noexcept{const D3D_SHADER_MACRO macros[]{{"DIGITOR_TEXTURE_OUTPUT","1"},{nullptr,nullptr}};return D3DCompile(digitor_primary_wheels_hlsl,sizeof(digitor_primary_wheels_hlsl),"primary_wheels.hlsl",macros,nullptr,"main","cs_5_1",D3DCOMPILE_OPTIMIZATION_LEVEL3,0,bytecode.ReleaseAndGetAddressOf(),errors.ReleaseAndGetAddressOf());}

struct D3DObject {
  ComPtr<ID3D12Resource> resource;
};
struct D3DPreviewOwner { ComPtr<ID3D12Resource> output,preview; D3D12_RESOURCE_STATES output_state{D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE};std::shared_ptr<void> upstream; };
struct D3DConsumerOwner { ComPtr<ID3D12Device> device;ComPtr<ID3D12Resource> destination; };
struct D3DPipelineBundle { ComPtr<ID3DBlob> shader;ComPtr<ID3D12RootSignature> root;ComPtr<ID3D12PipelineState> pipeline; };
...
public:
  ...
  std::shared_ptr<D3DPipelineBundle> color_pipeline(bool curves) noexcept {
    std::string identity=curves?"rgb-curves:cs5.1:root-v1":"primary-wheels:cs5.1:root-v1";if(gpu_failure_point()==GpuFailurePoint::PipelineCreation)identity += ":injected-create";NativePipelineCacheKey key{DIGITOR_RENDERER_D3D12,reinterpret_cast<std::uintptr_t>(device_.Get()),identity,1,GpuPrecisionMode::Float32,DIGITOR_PIXEL_FORMAT_RGBA32_FLOAT};
    return std::static_pointer_cast<D3DPipelineBundle>(pipeline_cache_.get_or_create(key,[&]()->NativePipelineCache::Object{
      if(gpu_failure_point()==GpuFailurePoint::ShaderCompilation){(void)injected_failure(GpuFailurePoint::ShaderCompilation);return {};}auto bundle=std::make_shared<D3DPipelineBundle>();ComPtr<ID3DBlob>error;
      if(FAILED(curves?compile_rgb_curves(true,bundle->shader,error):compile_primary_wheels(bundle->shader,error)))return {};
      D3D12_DESCRIPTOR_RANGE ranges[2]{};ranges[0]={D3D12_DESCRIPTOR_RANGE_TYPE_SRV,curves?2u:1u,0};ranges[1]={D3D12_DESCRIPTOR_RANGE_TYPE_UAV,1,0};D3D12_ROOT_PARAMETER roots[3]{};for(int n=0;n<2;n++){roots[n].ParameterType=D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;roots[n].DescriptorTable={1,&ranges[n]};}roots[2].ParameterType=D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;roots[2].Constants={0,0,curves?UINT(sizeof(NativeRgbCurvesParameters)/4):UINT(sizeof(NativePrimaryWheelsParameters)/4)};D3D12_ROOT_SIGNATURE_DESC rd{3,roots,0,nullptr,D3D12_ROOT_SIGNATURE_FLAG_NONE};ComPtr<ID3DBlob>sig,signature_error;if(FAILED(D3D12SerializeRootSignature(&rd,D3D_ROOT_SIGNATURE_VERSION_1,&sig,&signature_error))||FAILED(device_->CreateRootSignature(0,sig->GetBufferPointer(),sig->GetBufferSize(),IID_PPV_ARGS(&bundle->root))))return {};D3D12_COMPUTE_PIPELINE_STATE_DESC pd{};pd.pRootSignature=bundle->root.Get();pd.CS={bundle->shader->GetBufferPointer(),bundle->shader->GetBufferSize()};if(gpu_failure_point()==GpuFailurePoint::PipelineCreation){(void)injected_failure(GpuFailurePoint::PipelineCreation);return {};}if(FAILED(device_->CreateComputePipelineState(&pd,IID_PPV_ARGS(&bundle->pipeline))))return {};return std::static_pointer_cast<void>(bundle);
    }));
  }
  ...
  void shutdown() noexcept override {
    ...
    pipeline_cache_.invalidate_device(DIGITOR_RENDERER_D3D12,reinterpret_cast<std::uintptr_t>(device_.Get()));
    rtv_heap_.Reset();
    srv_heap_.Reset();
  ...
  DigitorResult execute_process_primary_wheels_gpu(std::span<const Color>source,uint32_t width,uint32_t height,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();begin_grade_provenance(DIGITOR_RENDERER_D3D12,true,info_.device_name,"D3DCompile cs_5_1","primary_wheels.hlsl:texture","ID3D12PipelineState:primary-wheels-v1");if(!width||!height||source.size()!=size_t(width)*height)return DIGITOR_RESULT_INVALID_ARGUMENT;auto cached=color_pipeline(false);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto root=cached->root;auto pipeline=cached->pipeline;constexpr UINT dwords=sizeof(NativePrimaryWheelsParameters)/4;
    auto committed=[&](const D3D12_RESOURCE_DESC&d,D3D12_HEAP_TYPE ht,D3D12_RESOURCE_STATES st,ComPtr<ID3D12Resource>&r){D3D12_HEAP_PROPERTIES hp{};hp.Type=ht;return device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&d,st,nullptr,IID_PPV_ARGS(&r));};D3D12_RESOURCE_DESC td{};td.Dimension=D3D12_RESOURCE_DIMENSION_TEXTURE2D;td.Width=width;td.Height=height;td.DepthOrArraySize=td.MipLevels=1;td.Format=DXGI_FORMAT_R32G32B32A32_FLOAT;td.SampleDesc.Count=1;ComPtr<ID3D12Resource>input;auto owner=std::make_shared<D3DPreviewOwner>();if(FAILED(committed(td,D3D12_HEAP_TYPE_DEFAULT,D3D12_RESOURCE_STATE_COPY_DEST,input)))return DIGITOR_RESULT_OUT_OF_MEMORY;td.Flags=D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;if(FAILED(committed(td,D3D12_HEAP_TYPE_DEFAULT,D3D12_RESOURCE_STATE_UNORDERED_ACCESS,owner->output)))return DIGITOR_RESULT_OUT_OF_MEMORY;td.Flags=D3D12_RESOURCE_FLAG_NONE;if(FAILED(committed(td,D3D12_HEAP_TYPE_DEFAULT,D3D12_RESOURCE_STATE_COPY_DEST,owner->preview)))return DIGITOR_RESULT_OUT_OF_MEMORY;
  ...
  DigitorResult execute_process_primary_wheels_gpu(const GpuSourceResource&s,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();auto prior=std::static_pointer_cast<D3DPreviewOwner>(native_owner(*s.frame));if(!prior||!prior->output||prior->output_state!=D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)return DIGITOR_RESULT_INVALID_ARGUMENT;auto cached=color_pipeline(false);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto root=cached->root;auto pipeline=cached->pipeline;constexpr UINT dwords=sizeof(NativePrimaryWheelsParameters)/4;
    auto owner=std::make_shared<D3DPreviewOwner>();owner->upstream=prior;D3D12_RESOURCE_DESC td=prior->output->GetDesc();if(td.Dimension!=D3D12_RESOURCE_DIMENSION_TEXTURE2D||td.Width!=s.width||td.Height!=s.height||td.Format!=DXGI_FORMAT_R32G32B32A32_FLOAT)return DIGITOR_RESULT_INVALID_ARGUMENT;D3D12_HEAP_PROPERTIES hp{};hp.Type=D3D12_HEAP_TYPE_DEFAULT;td.Flags=D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&td,D3D12_RESOURCE_STATE_UNORDERED_ACCESS,nullptr,IID_PPV_ARGS(&owner->output))))return DIGITOR_RESULT_OUT_OF_MEMORY;td.Flags=D3D12_RESOURCE_FLAG_NONE;if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&td,D3D12_RESOURCE_STATE_COPY_DEST,nullptr,IID_PPV_ARGS(&owner->preview))))return DIGITOR_RESULT_OUT_OF_MEMORY;
    D3D12_DESCRIPTOR_HEAP_DESC hd{D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,2,D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE};ComPtr<ID3D12DescriptorHeap>heap;if(FAILED(device_->CreateDescriptorHeap(&hd,IID_PPV_ARGS(&heap))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto cpu=heap->GetCPUDescriptorHandleForHeapStart();auto stride=device_->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);D3D12_SHADER_RESOURCE_VIEW_DESC sd{};sd.Format=DXGI_FORMAT_R32G32B32A32_FLOAT;sd.ViewDimension=D3D12_SRV_DIMENSION_TEXTURE2D;sd.Shader4ComponentMapping=D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;sd.Texture2D.MipLevels=1;device_->CreateShaderResourceView(prior->output.Get(),&sd,cpu);cpu.ptr+=stride;D3D12_UNORDERED_ACCESS_VIEW_DESC ud{};ud.Format=DXGI_FORMAT_R32G32B32A32_FLOAT;ud.ViewDimension=D3D12_UAV_DIMENSION_TEXTURE2D;device_->CreateUnorderedAccessView(owner->output.Get(),nullptr,&ud,cpu);
    allocator_->Reset();list_->Reset(allocator_.Get(),pipeline.Get());ID3D12DescriptorHeap*heaps[]{heap.Get()};list_->SetDescriptorHeaps(1,heaps);list_->SetComputeRootSignature(root.Get());auto gpu=heap->GetGPUDescriptorHandleForHeapStart();list_->SetComputeRootDescriptorTable(0,gpu);gpu.ptr+=stride;list_->SetComputeRootDescriptorTable(1,gpu);auto params=native_primary_wheels_parameters(parameters,s.width*s.height,s.width,s.height);list_->SetComputeRoot32BitConstants(2,dwords,&params,0);list_->Dispatch((s.width*s.height+63)/64,1,1);D3D12_RESOURCE_BARRIER b{};b.Type=D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;b.Transition={owner->output.Get(),0,D3D12_RESOURCE_STATE_UNORDERED_ACCESS,D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE};list_->ResourceBarrier(1,&b);if(FAILED(list_->Close()))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ID3D12CommandList*l[]{list_.Get()};queue_->ExecuteCommandLists(1,l);auto status=signal_and_wait();if(status!=DIGITOR_RESULT_OK)return status;static std::atomic_uint64_t ids{200000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_D3D12,GpuFrameMetadata{s.width,s.height,s.format,GpuFrameAlpha::straight,timestamp,s.color_metadata_identity},ids++,std::static_pointer_cast<void>(owner),std::make_shared<std::atomic_bool>(true),true);provenance_.primary_wheels_source_bound=provenance_.primary_wheels_destination_bound=provenance_.primary_wheels_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;return DIGITOR_RESULT_OK;
  }
  DigitorResult execute_validation_readback_primary_wheels(const ProcessedGpuFramePtr&frame,std::span<Color>out)noexcept override{
    if(!frame||frame->backend()!=DIGITOR_RENDERER_D3D12||out.size()!=std::size_t(frame->metadata().width)*frame->metadata().height)return DIGITOR_RESULT_INVALID_ARGUMENT;auto owner=std::static_pointer_cast<D3DPreviewOwner>(native_owner(*frame));if(!owner)return DIGITOR_RESULT_INVALID_ARGUMENT;const UINT64 bytes=out.size_bytes();D3D12_HEAP_PROPERTIES hp{};hp.Type=D3D12_HEAP_TYPE_READBACK;D3D12_RESOURCE_DESC d{};d.Dimension=D3D12_RESOURCE_DIMENSION_BUFFER;d.Width=bytes;d.Height=1;d.DepthOrArraySize=d.MipLevels=1;d.SampleDesc.Count=1;d.Layout=D3D12_TEXTURE_LAYOUT_ROW_MAJOR;ComPtr<ID3D12Resource>readback;if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&d,D3D12_RESOURCE_STATE_COPY_DEST,nullptr,IID_PPV_ARGS(&readback))))return DIGITOR_RESULT_OUT_OF_MEMORY;
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprint{};UINT rows{};UINT64 row_bytes{},total{};const auto td=owner->output->GetDesc();device_->GetCopyableFootprints(&td,0,1,0,&footprint,&rows,&row_bytes,&total);if(total>bytes){d.Width=total;readback.Reset();if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&d,D3D12_RESOURCE_STATE_COPY_DEST,nullptr,IID_PPV_ARGS(&readback))))return DIGITOR_RESULT_OUT_OF_MEMORY;}allocator_->Reset();list_->Reset(allocator_.Get(),nullptr);D3D12_RESOURCE_BARRIER b{};b.Type=D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;b.Transition={owner->output.Get(),0,D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,D3D12_RESOURCE_STATE_COPY_SOURCE};list_->ResourceBarrier(1,&b);D3D12_TEXTURE_COPY_LOCATION src{owner->output.Get(),D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX},dst{readback.Get(),D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT};dst.PlacedFootprint=footprint;list_->CopyTextureRegion(&dst,0,0,0,&src,nullptr);std::swap(b.Transition.StateBefore,b.Transition.StateAfter);list_->ResourceBarrier(1,&b);if(FAILED(list_->Close()))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ID3D12CommandList*l[]{list_.Get()};queue_->ExecuteCommandLists(1,l);if(signal_and_wait()!=DIGITOR_RESULT_OK)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;void*p=nullptr;D3D12_RANGE range{0,static_cast<SIZE_T>(total)};if(FAILED(readback->Map(0,&range,&p)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;const auto&m=frame->metadata();for(UINT y=0;y<m.height;y++)std::memcpy(out.data()+std::size_t(y)*m.width,static_cast<const std::byte*>(p)+footprint.Offset+std::size_t(y)*footprint.Footprint.RowPitch,std::size_t(m.width)*sizeof(Color));D3D12_RANGE none{0,0};readback->Unmap(0,&none);return DIGITOR_RESULT_OK;
  }
  DigitorResult execute_process_curves_gpu(std::span<const Color> source,uint32_t width,uint32_t height,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{
    ...
    auto cached=color_pipeline(true);if(!cached){provenance_.failure_stage="RGB Curves cached pipeline creation";return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}auto root=cached->root;auto pipeline=cached->pipeline;constexpr UINT dwords=sizeof(NativeRgbCurvesParameters)/4;
  ...
  DigitorResult execute_process_curves_gpu(const GpuSourceResource&s,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();auto prior=std::static_pointer_cast<D3DPreviewOwner>(native_owner(*s.frame));if(!prior||!prior->output||prior->output_state!=D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)return DIGITOR_RESULT_INVALID_ARGUMENT;D3D12_RESOURCE_DESC td=prior->output->GetDesc();if(td.Dimension!=D3D12_RESOURCE_DIMENSION_TEXTURE2D||td.Width!=s.width||td.Height!=s.height||td.Format!=DXGI_FORMAT_R32G32B32A32_FLOAT)return DIGITOR_RESULT_INVALID_ARGUMENT;
    constexpr UINT dwords=sizeof(NativeRgbCurvesParameters)/4;auto cached=color_pipeline(true);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto root=cached->root;auto pipeline=cached->pipeline;
    auto owner=std::make_shared<D3DPreviewOwner>();owner->upstream=prior;D3D12_HEAP_PROPERTIES hp{};hp.Type=D3D12_HEAP_TYPE_DEFAULT;td.Flags=D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&td,D3D12_RESOURCE_STATE_UNORDERED_ACCESS,nullptr,IID_PPV_ARGS(&owner->output))))return DIGITOR_RESULT_OUT_OF_MEMORY;td.Flags=D3D12_RESOURCE_FLAG_NONE;if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&td,D3D12_RESOURCE_STATE_COPY_DEST,nullptr,IID_PPV_ARGS(&owner->preview))))return DIGITOR_RESULT_OUT_OF_MEMORY;auto lut=native_rgb_curves_lut(curves);D3D12_HEAP_PROPERTIES up{};up.Type=D3D12_HEAP_TYPE_UPLOAD;D3D12_RESOURCE_DESC bd{};bd.Dimension=D3D12_RESOURCE_DIMENSION_BUFFER;bd.Width=lut.size()*sizeof(float);bd.Height=1;bd.DepthOrArraySize=bd.MipLevels=1;bd.SampleDesc.Count=1;bd.Layout=D3D12_TEXTURE_LAYOUT_ROW_MAJOR;ComPtr<ID3D12Resource>lb;if(FAILED(device_->CreateCommittedResource(&up,D3D12_HEAP_FLAG_NONE,&bd,D3D12_RESOURCE_STATE_GENERIC_READ,nullptr,IID_PPV_ARGS(&lb))))return DIGITOR_RESULT_OUT_OF_MEMORY;void*m=nullptr;D3D12_RANGE none{0,0};lb->Map(0,&none,&m);std::memcpy(m,lut.data(),lut.size()*sizeof(float));lb->Unmap(0,nullptr);
    D3D12_DESCRIPTOR_HEAP_DESC hd{D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,3,D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE};ComPtr<ID3D12DescriptorHeap>heap;if(FAILED(device_->CreateDescriptorHeap(&hd,IID_PPV_ARGS(&heap))))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto cpu=heap->GetCPUDescriptorHandleForHeapStart();auto stride=device_->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);D3D12_SHADER_RESOURCE_VIEW_DESC sd{};sd.Format=DXGI_FORMAT_R32G32B32A32_FLOAT;sd.ViewDimension=D3D12_SRV_DIMENSION_TEXTURE2D;sd.Shader4ComponentMapping=D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;sd.Texture2D.MipLevels=1;device_->CreateShaderResourceView(prior->output.Get(),&sd,cpu);cpu.ptr+=stride;sd={};sd.Format=DXGI_FORMAT_UNKNOWN;sd.ViewDimension=D3D12_SRV_DIMENSION_BUFFER;sd.Shader4ComponentMapping=D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;UINT lut_count{};if(!checked_uint(lut.size(),lut_count))return DIGITOR_RESULT_INVALID_ARGUMENT;sd.Buffer.NumElements=lut_count;sd.Buffer.StructureByteStride=sizeof(float);device_->CreateShaderResourceView(lb.Get(),&sd,cpu);cpu.ptr+=stride;D3D12_UNORDERED_ACCESS_VIEW_DESC ud{};ud.Format=DXGI_FORMAT_R32G32B32A32_FLOAT;ud.ViewDimension=D3D12_UAV_DIMENSION_TEXTURE2D;device_->CreateUnorderedAccessView(owner->output.Get(),nullptr,&ud,cpu);allocator_->Reset();list_->Reset(allocator_.Get(),pipeline.Get());ID3D12DescriptorHeap*heaps[]{heap.Get()};list_->SetDescriptorHeaps(1,heaps);list_->SetComputeRootSignature(root.Get());auto gpu=heap->GetGPUDescriptorHandleForHeapStart();list_->SetComputeRootDescriptorTable(0,gpu);gpu.ptr+=2*stride;list_->SetComputeRootDescriptorTable(1,gpu);auto params=native_rgb_curves_parameters(curves,s.width*s.height);params.padding[0]=s.width;params.padding[1]=s.height;list_->SetComputeRoot32BitConstants(2,dwords,&params,0);list_->Dispatch((s.width*s.height+63)/64,1,1);D3D12_RESOURCE_BARRIER b{};b.Type=D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;b.Transition={owner->output.Get(),0,D3D12_RESOURCE_STATE_UNORDERED_ACCESS,D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE};list_->ResourceBarrier(1,&b);if(FAILED(list_->Close()))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ID3D12CommandList*l[]{list_.Get()};queue_->ExecuteCommandLists(1,l);auto status=signal_and_wait();if(status!=DIGITOR_RESULT_OK)return status;static std::atomic_uint64_t ids{300000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_D3D12,GpuFrameMetadata{s.width,s.height,s.format,GpuFrameAlpha::straight,timestamp,s.color_metadata_identity},ids++,std::static_pointer_cast<void>(owner),std::make_shared<std::atomic_bool>(true),true);provenance_.curve_source_bound=provenance_.curve_destination_bound=provenance_.curve_lut_bound=provenance_.curve_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;return DIGITOR_RESULT_OK;
  }
  DigitorResult execute_create_preview_consumer(const ProcessedGpuFramePtr&frame,std::shared_ptr<PreviewConsumerDestination>&out)noexcept override{if(gpu_failure_point()==GpuFailurePoint::PreviewAcquisition)return injected_failure(GpuFailurePoint::PreviewAcquisition);out.reset();if(!frame||!device_)return DIGITOR_RESULT_NOT_INITIALIZED;const auto&m=frame->metadata();D3D12_RESOURCE_DESC d{};d.Dimension=D3D12_RESOURCE_DIMENSION_TEXTURE2D;d.Width=m.width;d.Height=m.height;d.DepthOrArraySize=d.MipLevels=1;d.Format=DXGI_FORMAT_R32G32B32A32_FLOAT;d.SampleDesc.Count=1;D3D12_HEAP_PROPERTIES hp{};hp.Type=D3D12_HEAP_TYPE_DEFAULT;auto owner=std::make_shared<D3DConsumerOwner>();owner->device=device_;if(FAILED(device_->CreateCommittedResource(&hp,D3D12_HEAP_FLAG_NONE,&d,D3D12_RESOURCE_STATE_COPY_DEST,nullptr,IID_PPV_ARGS(&owner->destination))))return DIGITOR_RESULT_OUT_OF_MEMORY;static std::atomic_uint64_t tokens{1};out=std::make_shared<PreviewConsumerDestination>(PreviewConsumerMetadata{DIGITOR_RENDERER_D3D12,this,m.width,m.height,m.format,GpuPrecisionMode::Float32},tokens++,std::static_pointer_cast<void>(owner),std::make_shared<std::atomic_bool>(true),[this](const ProcessedGpuFramePtr&f,const std::shared_ptr<void>&d){if(gpu_failure_point()==GpuFailurePoint::PreviewPresentation)return injected_failure(GpuFailurePoint::PreviewPresentation);auto source=std::static_pointer_cast<D3DPreviewOwner>(native_owner(*f));auto destination=std::static_pointer_cast<D3DConsumerOwner>(d);if(!source||!destination||destination->device.Get()!=device_.Get())return DIGITOR_RESULT_INVALID_ARGUMENT;if(FAILED(allocator_->Reset())||FAILED(list_->Reset(allocator_.Get(),nullptr)))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;D3D12_RESOURCE_BARRIER b[2]{};for(auto&x:b)x.Type=D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;b[0].Transition={source->output.Get(),0,D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,D3D12_RESOURCE_STATE_COPY_SOURCE};b[1].Transition={destination->destination.Get(),0,D3D12_RESOURCE_STATE_COPY_DEST,D3D12_RESOURCE_STATE_COPY_DEST};list_->ResourceBarrier(1,b);list_->CopyResource(destination->destination.Get(),source->output.Get());std::swap(b[0].Transition.StateBefore,b[0].Transition.StateAfter);b[1].Transition={destination->destination.Get(),0,D3D12_RESOURCE_STATE_COPY_DEST,D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE};list_->ResourceBarrier(2,b);if(FAILED(list_->Close()))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;ID3D12CommandList*l[]{list_.Get()};queue_->ExecuteCommandLists(1,l);return signal_and_wait();});return DIGITOR_RESULT_OK;}
  ...
  DigitorResult execute_curves_rgba32f(std::span<const Color> source,std::span<Color> out,
    ...
    if(gpu_failure_point()!=GpuFailurePoint::None&&gpu_failure_point()!=GpuFailurePoint::ShaderCompilation&&gpu_failure_point()!=GpuFailurePoint::PipelineCreation)return injected_failure(gpu_failure_point());if(source.size()!=out.size())return DIGITOR_RESULT_INVALID_ARGUMENT;if(source.empty())return DIGITOR_RESULT_OK;UINT source_count=0;if(!checked_uint(source.size(),source_count))return DIGITOR_RESULT_INVALID_ARGUMENT;
