...
public:
  ...
  DigitorResult execute_process_curves_gpu(std::span<const Color> source,uint32_t width,uint32_t height,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{
    out.reset();begin_grade_provenance(DIGITOR_RENDERER_D3D12,true,info_.device_name,shader_compiler_.identity().c_str(),"rgb_curves.hlsl:texture","D3D12 texture curves");if(!width||!height||source.size()!=size_t(width)*height)return DIGITOR_RESULT_INVALID_ARGUMENT;
    ShaderCompileRequest request{.source=digitor_rgb_curves_hlsl,.entry_point="main",.source_name="rgb_curves.hlsl",.target_profile="cs_6_0",.stage=ShaderStage::compute,.backend=ShaderBackend::d3d12,.macros={{"DIGITOR_TEXTURE_OUTPUT","1"}}};auto shader=shader_cache_.get_or_compile(shader_compiler_,request);if(!shader)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
