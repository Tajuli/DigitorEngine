...
public:
  ...
  std::shared_ptr<D3DPipelineBundle> color_pipeline(int operation) noexcept {
    const bool curves = operation == 1; const bool log_wheels = operation == 2;
    std::string identity =
        curves ? "rgb-curves:cs5.1:root-v1" : log_wheels ? "log-wheels:cs5.1:root-v1" : "primary-wheels:cs5.1:root-v1";
