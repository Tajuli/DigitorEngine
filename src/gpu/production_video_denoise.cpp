...
DenoiseResult apply_video_denoise_reference(const DenoiseFrame& cur,const DenoiseFrame* prev,const std::vector<float>* motion,DenoiseFrame& out,const DenoiseSettings& s){
 ...
 out={cur.width,cur.height,{}}; out.pixels.resize(n); double d=0;
 for(std::uint32_t y=0;y<cur.height;++y)for(std::uint32_t x=0;x<cur.width;++x){const std::size_t i=std::size_t(y)*cur.width+x; const auto c=cur.pixels[i]; DenoisePixel sum{}; float wsum=0;
  ...
  p.r=c01(p.r);p.g=c01(p.g);p.b=c01(p.b);p.a=c01(p.a);out.pixels[i]=p;d+=std::abs(p.r-c.r)+std::abs(p.g-c.g)+std::abs(p.b-c.b);
 }
...
extern "C" std::uint32_t digitor_video_denoise_rgba32f(const float* cur,const float* prev,const float* motion,float* out,std::uint32_t w,std::uint32_t h,const DigitorVideoDenoiseSettings* c,std::uint64_t* digest){if(!cur||!out||!c||!digest||w==0||h==0)return 1;const std::size_t n=std::size_t(w)*h;digitor::DenoiseFrame a{w,h,{}},b{w,h,{}};a.pixels.resize(n);if(prev)b.pixels.resize(n);std::vector<float> mv;if(motion)mv.assign(motion,motion+n);for(std::size_t i=0;i<n;++i){const auto k=i*4;a.pixels[i]={cur[k],cur[k+1],cur[k+2],cur[k+3]};if(prev)b.pixels[i]={prev[k],prev[k+1],prev[k+2],prev[k+3]};}digitor::DenoiseSettings s{c->spatial_strength,c->temporal_strength,c->luma_threshold,c->chroma_threshold,c->detail_preservation,c->motion_sensitivity};digitor::DenoiseFrame o;const auto r=digitor::apply_video_denoise_reference(a,prev?&b:nullptr,motion?&mv:nullptr,o,s);if(r.status!=digitor::DenoiseStatus::ready)return 2;for(std::size_t i=0;i<n;++i){const auto k=i*4;out[k]=o.pixels[i].r;out[k+1]=o.pixels[i].g;out[k+2]=o.pixels[i].b;out[k+3]=o.pixels[i].a;}*digest=r.digest;return 0;}
