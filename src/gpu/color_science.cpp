...
Mat3 rgb_to_xyz(const Chromaticities&p){auto xyz=[](Chromaticity c){if(c.x<=0||c.y<=0||c.x+c.y>1)throw std::invalid_argument("invalid primary chromaticity");return Vec3{c.x/c.y,1,(1-c.x-c.y)/c.y};};auto r=xyz(p.red),g=xyz(p.green),b=xyz(p.blue);Mat3 base{{{r.x,g.x,b.x,r.y,g.y,b.y,r.z,g.z,b.z}}};auto s=multiply(inverse(base),white_xyz(p.white));return multiply(base,diagonal(s));}
