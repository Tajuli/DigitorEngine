...
DigitorResult IRenderBackend::process_log_wheels_gpu(std::span<const Color>s,std::uint32_t w,std::uint32_t h,std::int64_t ts,const LogWheelsParameters&p,ProcessedGpuFramePtr&out)noexcept{
 out.reset();const auto before=log_wheels_reference_count();auto result=execute_process_log_wheels_gpu(s,w,h,ts,p,out);provenance_.cpu_log_wheels_invocations=log_wheels_reference_count()-before;if(result!=DIGITOR_RESULT_OK||!out||provenance_.cpu_log_wheels_invocations||provenance_.log_wheels_fallback_invocations||provenance_.normal_preview_readback_count){out.reset();return result==DIGITOR_RESULT_OK?DIGITOR_RESULT_INTERNAL_ERROR:result;}out->bind_context_lifetime(context_lifetime_);provenance_.preview_source=PreviewSource::gpu;return DIGITOR_RESULT_OK;
}
DigitorResult IRenderBackend::process_log_wheels_gpu(const GpuSourceResource&s,std::int64_t ts,const LogWheelsParameters&p,ProcessedGpuFramePtr&out)noexcept{
 out.reset();if(!s.usable_by(info().backend,context_identity_))return DIGITOR_RESULT_INVALID_ARGUMENT;const auto before=log_wheels_reference_count();auto result=execute_process_log_wheels_gpu(s,ts,p,out);provenance_.cpu_log_wheels_invocations=log_wheels_reference_count()-before;if(result!=DIGITOR_RESULT_OK||!out||provenance_.cpu_log_wheels_invocations||provenance_.log_wheels_fallback_invocations||provenance_.normal_preview_readback_count){out.reset();return result==DIGITOR_RESULT_OK?DIGITOR_RESULT_INTERNAL_ERROR:result;}out->bind_context_lifetime(context_lifetime_);return DIGITOR_RESULT_OK;
}
DigitorResult IRenderBackend::validation_readback_log_wheels(const ProcessedGpuFramePtr&f,std::span<Color>o)noexcept{if(!f||!f->validation_readback_supported())return DIGITOR_RESULT_UNSUPPORTED;auto r=execute_validation_readback_log_wheels(f,o);provenance_.validation_readback_completed=r==DIGITOR_RESULT_OK;return r;}
DigitorResult IRenderBackend::execute_process_log_wheels_gpu(std::span<const Color>,std::uint32_t,std::uint32_t,std::int64_t,const LogWheelsParameters&,ProcessedGpuFramePtr&o)noexcept{o.reset();return DIGITOR_RESULT_UNSUPPORTED;}
DigitorResult IRenderBackend::execute_process_log_wheels_gpu(const GpuSourceResource&,std::int64_t,const LogWheelsParameters&,ProcessedGpuFramePtr&o)noexcept{o.reset();return DIGITOR_RESULT_UNSUPPORTED;}
DigitorResult IRenderBackend::execute_validation_readback_log_wheels(const ProcessedGpuFramePtr&,std::span<Color>)noexcept{return DIGITOR_RESULT_UNSUPPORTED;}

DigitorResult IRenderBackend::process_hsl_qualifier_gpu(std::span<const Color>s,std::uint32_t w,std::uint32_t h,std::int64_t ts,const HslQualifierParameters&p,ProcessedGpuFramePtr&out)noexcept{
 out.reset();if(!w||!h||s.size()!=static_cast<std::size_t>(w)*h)return DIGITOR_RESULT_INVALID_ARGUMENT;const auto before=hsl_qualifier_reference_count();auto result=execute_process_hsl_qualifier_gpu(s,w,h,ts,p,out);if(result!=DIGITOR_RESULT_OK||!out||hsl_qualifier_reference_count()!=before||provenance_.normal_preview_readback_count){out.reset();return result==DIGITOR_RESULT_OK?DIGITOR_RESULT_INTERNAL_ERROR:result;}out->bind_context_lifetime(context_lifetime_);provenance_.preview_source=PreviewSource::gpu;return DIGITOR_RESULT_OK;
}
DigitorResult IRenderBackend::process_hsl_qualifier_gpu(const GpuSourceResource&s,std::int64_t ts,const HslQualifierParameters&p,ProcessedGpuFramePtr&out)noexcept{
 out.reset();if(!s.usable_by(info().backend,context_identity_))return DIGITOR_RESULT_INVALID_ARGUMENT;const auto before=hsl_qualifier_reference_count();auto result=execute_process_hsl_qualifier_gpu(s,ts,p,out);if(result!=DIGITOR_RESULT_OK||!out||hsl_qualifier_reference_count()!=before||provenance_.normal_preview_readback_count){out.reset();return result==DIGITOR_RESULT_OK?DIGITOR_RESULT_INTERNAL_ERROR:result;}out->bind_context_lifetime(context_lifetime_);provenance_.preview_source=PreviewSource::gpu;return DIGITOR_RESULT_OK;
}
DigitorResult IRenderBackend::validation_readback_hsl_qualifier(const ProcessedGpuFramePtr&f,std::span<float>o)noexcept{if(!f||!f->validation_readback_supported())return DIGITOR_RESULT_UNSUPPORTED;auto r=execute_validation_readback_hsl_qualifier(f,o);provenance_.validation_readback_completed=r==DIGITOR_RESULT_OK;return r;}
DigitorResult IRenderBackend::execute_process_hsl_qualifier_gpu(std::span<const Color>,std::uint32_t,std::uint32_t,std::int64_t,const HslQualifierParameters&,ProcessedGpuFramePtr&o)noexcept{o.reset();return DIGITOR_RESULT_UNSUPPORTED;}
DigitorResult IRenderBackend::execute_process_hsl_qualifier_gpu(const GpuSourceResource&,std::int64_t,const HslQualifierParameters&,ProcessedGpuFramePtr&o)noexcept{o.reset();return DIGITOR_RESULT_UNSUPPORTED;}
DigitorResult IRenderBackend::execute_validation_readback_hsl_qualifier(const ProcessedGpuFramePtr&,std::span<float>)noexcept{return DIGITOR_RESULT_UNSUPPORTED;}
