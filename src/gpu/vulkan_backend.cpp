...
public:
  ...
  DigitorResult execute_process_primary_wheels_gpu(
    ...
    ShaderCompileRequest request{
        ...
        .debug_info = false};
    auto binary = shader_cache_.get_or_compile(shader_compiler_, request);
    if (!binary)
  ...
  DigitorResult
  execute_process_curves_gpu(std::span<const Color> src, uint32_t width,
  ...
  DigitorResult grade_rgba32f(std::span<const Color> src, std::span<Color> out,
    ...
    const auto grade_binary =
        shader_cache_.get_or_compile(shader_compiler_, grade_request);
    if (!grade_binary) {
      cleanup();
