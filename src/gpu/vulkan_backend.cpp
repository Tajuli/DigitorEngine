...
struct VkPreviewOwner { VkDevice device{};VkImage source{},output{},preview{};VkDeviceMemory source_memory{},output_memory{},preview_memory{};VkImageView source_view{},output_view{};VkImageLayout output_layout{VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};std::shared_ptr<void> upstream;std::weak_ptr<std::atomic_bool> device_live;
 ~VkPreviewOwner(){auto live=device_live.lock();if(!device||!live||!live->load(std::memory_order_acquire))return;if(source_view)vkDestroyImageView(device,source_view,nullptr);if(output_view)vkDestroyImageView(device,output_view,nullptr);for(auto i:{source,output,preview})if(i)vkDestroyImage(device,i,nullptr);for(auto m:{source_memory,output_memory,preview_memory})if(m)vkFreeMemory(device,m,nullptr);}
};
struct VkConsumerOwner { VkDevice device{};VkImage image{};VkImageView view{};VkDeviceMemory memory{};std::weak_ptr<std::atomic_bool> device_live;
 ~VkConsumerOwner(){auto live=device_live.lock();if(device&&live&&live->load(std::memory_order_acquire)){if(view)vkDestroyImageView(device,view,nullptr);if(image)vkDestroyImage(device,image,nullptr);if(memory)vkFreeMemory(device,memory,nullptr);}}
};
struct VkPipelineOwner { VkDevice device{};VkShaderModule shader{};VkDescriptorSetLayout descriptors{};VkPipelineLayout layout{};VkPipeline pipeline{};std::weak_ptr<std::atomic_bool> device_live;
 ~VkPipelineOwner(){auto live=device_live.lock();if(!device||!live||!live->load())return;if(pipeline)vkDestroyPipeline(device,pipeline,nullptr);if(shader)vkDestroyShaderModule(device,shader,nullptr);if(layout)vkDestroyPipelineLayout(device,layout,nullptr);if(descriptors)vkDestroyDescriptorSetLayout(device,descriptors,nullptr);}
};
class VulkanBackend final : public IRenderBackend {
  ...
  std::shared_ptr<std::atomic_bool> device_live_{std::make_shared<std::atomic_bool>(true)};
  DigitorRendererInfo i_{};
  NativePipelineCache pipeline_cache_{8};
  std::shared_ptr<VkPipelineOwner> color_pipeline(bool curves,const ShaderCompileResult&binary)noexcept{
    std::string identity=(curves?"rgb-curves:spirv-layout-v1:":"primary-wheels:spirv-layout-v1:")+binary.cache_key;
    if(gpu_failure_point()==GpuFailurePoint::PipelineCreation)identity += ":injected-create";NativePipelineCacheKey key{DIGITOR_RENDERER_VULKAN,reinterpret_cast<std::uintptr_t>(d_),identity,1,GpuPrecisionMode::Float32,DIGITOR_PIXEL_FORMAT_RGBA32_FLOAT};
    return std::static_pointer_cast<VkPipelineOwner>(pipeline_cache_.get_or_create(key,[&]()->NativePipelineCache::Object{
      if(gpu_failure_point()==GpuFailurePoint::ShaderCompilation){(void)injected_failure(GpuFailurePoint::ShaderCompilation);return {};}auto p=std::make_shared<VkPipelineOwner>();p->device=d_;p->device_live=device_live_;VkDescriptorSetLayoutBinding b[4]{{0,VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr},{1,VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr},{2,curves?VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr},{3,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr}};VkDescriptorSetLayoutCreateInfo di{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO};di.bindingCount=curves?4u:3u;di.pBindings=b;if(vkCreateDescriptorSetLayout(d_,&di,nullptr,&p->descriptors)!=VK_SUCCESS)return {};VkPipelineLayoutCreateInfo li{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};li.setLayoutCount=1;li.pSetLayouts=&p->descriptors;if(vkCreatePipelineLayout(d_,&li,nullptr,&p->layout)!=VK_SUCCESS)return {};VkShaderModuleCreateInfo si{VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};si.codeSize=binary.binary.size();si.pCode=reinterpret_cast<const uint32_t*>(binary.binary.data());if(vkCreateShaderModule(d_,&si,nullptr,&p->shader)!=VK_SUCCESS)return {};VkComputePipelineCreateInfo ci{VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO};ci.stage={VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO};ci.stage.stage=VK_SHADER_STAGE_COMPUTE_BIT;ci.stage.module=p->shader;ci.stage.pName="main";ci.layout=p->layout;if(gpu_failure_point()==GpuFailurePoint::PipelineCreation){(void)injected_failure(GpuFailurePoint::PipelineCreation);return {};}if(vkCreateComputePipelines(d_,VK_NULL_HANDLE,1,&ci,nullptr,&p->pipeline)!=VK_SUCCESS)return {};return std::static_pointer_cast<void>(p);
    }));
  }
...
public:
  ...
  void shutdown() noexcept override {
    if (d_)
      vkDeviceWaitIdle(d_);
    pipeline_cache_.invalidate_device(DIGITOR_RENDERER_VULKAN,reinterpret_cast<std::uintptr_t>(d_));
  }
  ...
  DigitorResult execute_process_primary_wheels_gpu(std::span<const Color>src,uint32_t width,uint32_t height,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{if(gpu_failure_point()!=GpuFailurePoint::None&&gpu_failure_point()!=GpuFailurePoint::ShaderCompilation&&gpu_failure_point()!=GpuFailurePoint::PipelineCreation)return injected_failure(gpu_failure_point());
    out.reset();begin_grade_provenance(DIGITOR_RENDERER_VULKAN,true,i_.device_name,shader_compiler_.identity().c_str(),"primary_wheels.hlsl:texture","VkPipeline:primary-wheels-v1");if(!width||!height||src.size()!=size_t(width)*height)return DIGITOR_RESULT_INVALID_ARGUMENT;
    ShaderCompileRequest request{.source=digitor_primary_wheels_hlsl,.entry_point="main",.source_name="primary_wheels.hlsl",.target_profile="cs_6_0",.stage=ShaderStage::compute,.backend=ShaderBackend::vulkan,.macros={{"DIGITOR_VULKAN","1"},{"DIGITOR_TEXTURE_OUTPUT","1"}},.include_roots={},.specialization_constants={},.optimization=ShaderOptimization::performance,.debug_info=false};auto binary=shader_cache_.get_or_compile(shader_compiler_,request);if(!binary)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto owner=std::shared_ptr<VkPreviewOwner>(new(std::nothrow)VkPreviewOwner{});if(!owner)return DIGITOR_RESULT_OUT_OF_MEMORY;owner->device=d_;owner->device_live=device_live_;auto image=[&](VkImageUsageFlags usage,VkImage&i,VkDeviceMemory&m){VkImageCreateInfo c{VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};c.imageType=VK_IMAGE_TYPE_2D;c.extent={width,height,1};c.mipLevels=c.arrayLayers=1;c.format=VK_FORMAT_R32G32B32A32_SFLOAT;c.tiling=VK_IMAGE_TILING_OPTIMAL;c.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;c.samples=VK_SAMPLE_COUNT_1_BIT;c.sharingMode=VK_SHARING_MODE_EXCLUSIVE;c.usage=usage;if(vkCreateImage(d_,&c,nullptr,&i)!=VK_SUCCESS)return false;VkMemoryRequirements r{};vkGetImageMemoryRequirements(d_,i,&r);auto mt=mem(r.memoryTypeBits,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);VkMemoryAllocateInfo a{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};a.allocationSize=r.size;a.memoryTypeIndex=mt;return mt!=UINT32_MAX&&vkAllocateMemory(d_,&a,nullptr,&m)==VK_SUCCESS&&vkBindImageMemory(d_,i,m,0)==VK_SUCCESS;};if(!image(VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->source,owner->source_memory)||!image(VK_IMAGE_USAGE_STORAGE_BIT|VK_IMAGE_USAGE_TRANSFER_SRC_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->output,owner->output_memory)||!image(VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->preview,owner->preview_memory))return DIGITOR_RESULT_OUT_OF_MEMORY;
    ...
    auto params=native_primary_wheels_parameters(parameters,uint32_t(src.size()),width,height);VkBuffer buffers[2]{};VkDeviceMemory memory[2]{};VkDeviceSize sizes[]{src.size_bytes(),sizeof(params)};auto clean_buffers=[&]{for(int n=0;n<2;n++){if(buffers[n])vkDestroyBuffer(d_,buffers[n],nullptr);if(memory[n])vkFreeMemory(d_,memory[n],nullptr);}};for(int n=0;n<2;n++){VkBufferCreateInfo c{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};c.size=sizes[n];c.usage=n?VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT:VK_BUFFER_USAGE_TRANSFER_SRC_BIT;c.sharingMode=VK_SHARING_MODE_EXCLUSIVE;if(vkCreateBuffer(d_,&c,nullptr,&buffers[n])!=VK_SUCCESS){clean_buffers();return DIGITOR_RESULT_OUT_OF_MEMORY;}VkMemoryRequirements r{};vkGetBufferMemoryRequirements(d_,buffers[n],&r);auto mt=mem(r.memoryTypeBits,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);VkMemoryAllocateInfo a{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};a.allocationSize=r.size;a.memoryTypeIndex=mt;if(mt==UINT32_MAX||vkAllocateMemory(d_,&a,nullptr,&memory[n])!=VK_SUCCESS||vkBindBufferMemory(d_,buffers[n],memory[n],0)!=VK_SUCCESS){clean_buffers();return DIGITOR_RESULT_OUT_OF_MEMORY;}void*m=nullptr;vkMapMemory(d_,memory[n],0,sizes[n],0,&m);std::memcpy(m,n?static_cast<void*>(&params):static_cast<const void*>(src.data()),sizes[n]);vkUnmapMemory(d_,memory[n]);}
    auto cached=color_pipeline(false,binary);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;VkDescriptorSetLayout sl=cached->descriptors;VkPipelineLayout pl=cached->layout;VkPipeline pipe=cached->pipeline;VkDescriptorPool dp{};VkCommandBuffer cmd{};auto finish=[&]{if(cmd)vkFreeCommandBuffers(d_,pool_,1,&cmd);if(dp)vkDestroyDescriptorPool(d_,dp,nullptr);clean_buffers();};
    VkDescriptorPoolSize ps[]{{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1},{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1},{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1}};VkDescriptorPoolCreateInfo di{VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO};di.maxSets=1;di.poolSizeCount=3;di.pPoolSizes=ps;vkCreateDescriptorPool(d_,&di,nullptr,&dp);VkDescriptorSetAllocateInfo ai{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};ai.descriptorPool=dp;ai.descriptorSetCount=1;ai.pSetLayouts=&sl;VkDescriptorSet set{};if(vkAllocateDescriptorSets(d_,&ai,&set)!=VK_SUCCESS){finish();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorImageInfo ii[2]{{VK_NULL_HANDLE,owner->source_view,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},{VK_NULL_HANDLE,owner->output_view,VK_IMAGE_LAYOUT_GENERAL}};VkDescriptorBufferInfo bi{buffers[1],0,sizeof(params)};VkWriteDescriptorSet w[3]{};for(int n=0;n<3;n++){w[n]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};w[n].dstSet=set;w[n].dstBinding=n;w[n].descriptorCount=1;}w[0].descriptorType=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;w[0].pImageInfo=&ii[0];w[1].descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;w[1].pImageInfo=&ii[1];w[2].descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w[2].pBufferInfo=&bi;vkUpdateDescriptorSets(d_,3,w,0,nullptr);
    VkCommandBufferAllocateInfo ca{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};ca.commandPool=pool_;ca.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ca.commandBufferCount=1;if(vkAllocateCommandBuffers(d_,&ca,&cmd)!=VK_SUCCESS){finish();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkCommandBufferBeginInfo cb{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};vkBeginCommandBuffer(cmd,&cb);auto barrier=[&](VkImage image,VkImageLayout old,VkImageLayout now,VkAccessFlags sa,VkAccessFlags da,VkPipelineStageFlags ss,VkPipelineStageFlags ds){VkImageMemoryBarrier x{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};x.oldLayout=old;x.newLayout=now;x.srcAccessMask=sa;x.dstAccessMask=da;x.image=image;x.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};vkCmdPipelineBarrier(cmd,ss,ds,0,0,nullptr,0,nullptr,1,&x);};barrier(owner->source,VK_IMAGE_LAYOUT_UNDEFINED,VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,0,VK_ACCESS_TRANSFER_WRITE_BIT,VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT);VkBufferImageCopy copy{};copy.imageSubresource={VK_IMAGE_ASPECT_COLOR_BIT,0,0,1};copy.imageExtent={width,height,1};vkCmdCopyBufferToImage(cmd,buffers[0],owner->source,VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,1,&copy);barrier(owner->source,VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,VK_ACCESS_TRANSFER_WRITE_BIT,VK_ACCESS_SHADER_READ_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);barrier(owner->output,VK_IMAGE_LAYOUT_UNDEFINED,VK_IMAGE_LAYOUT_GENERAL,0,VK_ACCESS_SHADER_WRITE_BIT,VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,pipe);vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,pl,0,1,&set,0,nullptr);vkCmdDispatch(cmd,(uint32_t(src.size())+63)/64,1,1);barrier(owner->output,VK_IMAGE_LAYOUT_GENERAL,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,VK_ACCESS_SHADER_WRITE_BIT,VK_ACCESS_SHADER_READ_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT|VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);vkEndCommandBuffer(cmd);VkSubmitInfo sub{VK_STRUCTURE_TYPE_SUBMIT_INFO};sub.commandBufferCount=1;sub.pCommandBuffers=&cmd;auto result=vkQueueSubmit(queue_,1,&sub,VK_NULL_HANDLE);if(result==VK_SUCCESS)result=vkQueueWaitIdle(queue_);finish();if(result!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    static std::atomic_uint64_t ids{100000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_VULKAN,GpuFrameMetadata{width,height,DIGITOR_PIXEL_FORMAT_RGBA32_FLOAT,GpuFrameAlpha::straight,timestamp,"linear-rgba"},ids++,std::static_pointer_cast<void>(owner),std::make_shared<std::atomic_bool>(true),true);provenance_.primary_wheels_enabled=true;provenance_.primary_wheels_parameter_identity=parameters.identity();provenance_.primary_wheels_shader_identity="primary_wheels.hlsl:SPIR-V-v1";provenance_.primary_wheels_pipeline_identity="VkPipeline:primary-wheels-v1";provenance_.primary_wheels_source_bound=provenance_.primary_wheels_destination_bound=provenance_.primary_wheels_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;return DIGITOR_RESULT_OK;
  }
  DigitorResult execute_process_primary_wheels_gpu(const GpuSourceResource&s,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{if(gpu_failure_point()!=GpuFailurePoint::None&&gpu_failure_point()!=GpuFailurePoint::ShaderCompilation&&gpu_failure_point()!=GpuFailurePoint::PipelineCreation)return injected_failure(gpu_failure_point());
    out.reset();auto prior=std::static_pointer_cast<VkPreviewOwner>(native_owner(*s.frame));if(!prior||prior->device!=d_||!prior->output||!prior->output_view||prior->output_layout!=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)return DIGITOR_RESULT_INVALID_ARGUMENT;ShaderCompileRequest request{.source=digitor_primary_wheels_hlsl,.entry_point="main",.source_name="primary_wheels.hlsl",.target_profile="cs_6_0",.stage=ShaderStage::compute,.backend=ShaderBackend::vulkan,.macros={{"DIGITOR_VULKAN","1"},{"DIGITOR_TEXTURE_OUTPUT","1"}},.include_roots={},.specialization_constants={},.optimization=ShaderOptimization::performance,.debug_info=false};auto binary=shader_cache_.get_or_compile(shader_compiler_,request);if(!binary)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto owner=std::make_shared<VkPreviewOwner>();owner->device=d_;owner->device_live=device_live_;owner->upstream=prior;auto image=[&](VkImageUsageFlags usage,VkImage&i,VkDeviceMemory&m){VkImageCreateInfo c{VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};c.imageType=VK_IMAGE_TYPE_2D;c.extent={s.width,s.height,1};c.mipLevels=c.arrayLayers=1;c.format=VK_FORMAT_R32G32B32A32_SFLOAT;c.tiling=VK_IMAGE_TILING_OPTIMAL;c.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;c.samples=VK_SAMPLE_COUNT_1_BIT;c.sharingMode=VK_SHARING_MODE_EXCLUSIVE;c.usage=usage;if(vkCreateImage(d_,&c,nullptr,&i)!=VK_SUCCESS)return false;VkMemoryRequirements r{};vkGetImageMemoryRequirements(d_,i,&r);auto mt=mem(r.memoryTypeBits,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);VkMemoryAllocateInfo a{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};a.allocationSize=r.size;a.memoryTypeIndex=mt;return mt!=UINT32_MAX&&vkAllocateMemory(d_,&a,nullptr,&m)==VK_SUCCESS&&vkBindImageMemory(d_,i,m,0)==VK_SUCCESS;};if(!image(VK_IMAGE_USAGE_STORAGE_BIT|VK_IMAGE_USAGE_TRANSFER_SRC_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->output,owner->output_memory)||!image(VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->preview,owner->preview_memory))return DIGITOR_RESULT_OUT_OF_MEMORY;VkImageViewCreateInfo vi{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};vi.image=owner->output;vi.viewType=VK_IMAGE_VIEW_TYPE_2D;vi.format=VK_FORMAT_R32G32B32A32_SFLOAT;vi.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};if(vkCreateImageView(d_,&vi,nullptr,&owner->output_view)!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto params=native_primary_wheels_parameters(parameters,s.width*s.height,s.width,s.height);VkBuffer pb{};VkDeviceMemory pm{};VkBufferCreateInfo bc{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};bc.size=sizeof(params);bc.usage=VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;if(vkCreateBuffer(d_,&bc,nullptr,&pb)!=VK_SUCCESS)return DIGITOR_RESULT_OUT_OF_MEMORY;VkMemoryRequirements mr{};vkGetBufferMemoryRequirements(d_,pb,&mr);auto mt=mem(mr.memoryTypeBits,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);VkMemoryAllocateInfo ma{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};ma.allocationSize=mr.size;ma.memoryTypeIndex=mt;if(mt==UINT32_MAX||vkAllocateMemory(d_,&ma,nullptr,&pm)!=VK_SUCCESS||vkBindBufferMemory(d_,pb,pm,0)!=VK_SUCCESS)return DIGITOR_RESULT_OUT_OF_MEMORY;void*m=nullptr;vkMapMemory(d_,pm,0,sizeof(params),0,&m);std::memcpy(m,&params,sizeof(params));vkUnmapMemory(d_,pm);
    auto cached=color_pipeline(false,binary);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;VkDescriptorSetLayout sl=cached->descriptors;VkPipelineLayout pl=cached->layout;VkPipeline pipe=cached->pipeline;VkDescriptorPool dp{};VkCommandBuffer cmd{};auto done=[&]{if(cmd)vkFreeCommandBuffers(d_,pool_,1,&cmd);if(dp)vkDestroyDescriptorPool(d_,dp,nullptr);if(pb)vkDestroyBuffer(d_,pb,nullptr);if(pm)vkFreeMemory(d_,pm,nullptr);};VkDescriptorPoolSize ps[]{{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1},{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1},{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1}};VkDescriptorPoolCreateInfo di{VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO};di.maxSets=1;di.poolSizeCount=3;di.pPoolSizes=ps;if(vkCreateDescriptorPool(d_,&di,nullptr,&dp)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorSetAllocateInfo ai{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};ai.descriptorPool=dp;ai.descriptorSetCount=1;ai.pSetLayouts=&sl;VkDescriptorSet set{};if(vkAllocateDescriptorSets(d_,&ai,&set)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorImageInfo ii[2]{{VK_NULL_HANDLE,prior->output_view,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},{VK_NULL_HANDLE,owner->output_view,VK_IMAGE_LAYOUT_GENERAL}};VkDescriptorBufferInfo bi{pb,0,sizeof(params)};VkWriteDescriptorSet w[3]{};for(int n=0;n<3;n++){w[n]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};w[n].dstSet=set;w[n].dstBinding=n;w[n].descriptorCount=1;}w[0].descriptorType=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;w[0].pImageInfo=&ii[0];w[1].descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;w[1].pImageInfo=&ii[1];w[2].descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w[2].pBufferInfo=&bi;vkUpdateDescriptorSets(d_,3,w,0,nullptr);
    VkCommandBufferAllocateInfo ca{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};ca.commandPool=pool_;ca.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ca.commandBufferCount=1;if(vkAllocateCommandBuffers(d_,&ca,&cmd)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkCommandBufferBeginInfo cb{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};vkBeginCommandBuffer(cmd,&cb);VkImageMemoryBarrier bars[2]{};for(auto&b:bars){b={VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};b.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};}bars[0].image=prior->output;bars[0].oldLayout=bars[0].newLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;bars[0].srcAccessMask=VK_ACCESS_SHADER_WRITE_BIT;bars[0].dstAccessMask=VK_ACCESS_SHADER_READ_BIT;bars[1].image=owner->output;bars[1].oldLayout=VK_IMAGE_LAYOUT_UNDEFINED;bars[1].newLayout=VK_IMAGE_LAYOUT_GENERAL;bars[1].dstAccessMask=VK_ACCESS_SHADER_WRITE_BIT;vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,0,0,nullptr,0,nullptr,2,bars);vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,pipe);vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,pl,0,1,&set,0,nullptr);vkCmdDispatch(cmd,(s.width*s.height+63)/64,1,1);bars[1].oldLayout=VK_IMAGE_LAYOUT_GENERAL;bars[1].newLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;bars[1].srcAccessMask=VK_ACCESS_SHADER_WRITE_BIT;bars[1].dstAccessMask=VK_ACCESS_SHADER_READ_BIT;vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT|VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,0,0,nullptr,0,nullptr,1,&bars[1]);vkEndCommandBuffer(cmd);VkSubmitInfo sub{VK_STRUCTURE_TYPE_SUBMIT_INFO};sub.commandBufferCount=1;sub.pCommandBuffers=&cmd;auto vr=vkQueueSubmit(queue_,1,&sub,VK_NULL_HANDLE);if(vr==VK_SUCCESS)vr=vkQueueWaitIdle(queue_);done();if(vr!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;static std::atomic_uint64_t ids{200000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_VULKAN,GpuFrameMetadata{s.width,s.height,s.format,GpuFrameAlpha::straight,timestamp,s.color_metadata_identity},ids++,std::static_pointer_cast<void>(owner),std::make_shared<std::atomic_bool>(true),true);provenance_.primary_wheels_source_bound=provenance_.primary_wheels_destination_bound=provenance_.primary_wheels_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;return DIGITOR_RESULT_OK;
  }
  DigitorResult execute_validation_readback_primary_wheels(const ProcessedGpuFramePtr&frame,std::span<Color>out)noexcept override{
    if(!frame||out.size()!=size_t(frame->metadata().width)*frame->metadata().height)return DIGITOR_RESULT_INVALID_ARGUMENT;auto owner=std::static_pointer_cast<VkPreviewOwner>(native_owner(*frame));if(!owner)return DIGITOR_RESULT_INVALID_ARGUMENT;VkDeviceSize bytes=out.size_bytes();VkBuffer buffer{};VkDeviceMemory memory{};VkBufferCreateInfo bc{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};bc.size=bytes;bc.usage=VK_BUFFER_USAGE_TRANSFER_DST_BIT;bc.sharingMode=VK_SHARING_MODE_EXCLUSIVE;if(vkCreateBuffer(d_,&bc,nullptr,&buffer)!=VK_SUCCESS)return DIGITOR_RESULT_OUT_OF_MEMORY;VkMemoryRequirements mr{};vkGetBufferMemoryRequirements(d_,buffer,&mr);auto mt=mem(mr.memoryTypeBits,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);VkMemoryAllocateInfo ma{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};ma.allocationSize=mr.size;ma.memoryTypeIndex=mt;if(mt==UINT32_MAX||vkAllocateMemory(d_,&ma,nullptr,&memory)!=VK_SUCCESS||vkBindBufferMemory(d_,buffer,memory,0)!=VK_SUCCESS){vkDestroyBuffer(d_,buffer,nullptr);if(memory)vkFreeMemory(d_,memory,nullptr);return DIGITOR_RESULT_OUT_OF_MEMORY;}VkCommandBuffer cmd{};VkCommandBufferAllocateInfo ca{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};ca.commandPool=pool_;ca.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ca.commandBufferCount=1;vkAllocateCommandBuffers(d_,&ca,&cmd);VkCommandBufferBeginInfo cb{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};vkBeginCommandBuffer(cmd,&cb);VkImageMemoryBarrier b{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};b.oldLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;b.newLayout=VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;b.srcAccessMask=VK_ACCESS_SHADER_READ_BIT;b.dstAccessMask=VK_ACCESS_TRANSFER_READ_BIT;b.image=owner->output;b.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,0,0,nullptr,0,nullptr,1,&b);VkBufferImageCopy copy{};copy.imageSubresource={VK_IMAGE_ASPECT_COLOR_BIT,0,0,1};copy.imageExtent={frame->metadata().width,frame->metadata().height,1};vkCmdCopyImageToBuffer(cmd,owner->output,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,buffer,1,&copy);std::swap(b.oldLayout,b.newLayout);std::swap(b.srcAccessMask,b.dstAccessMask);vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,0,0,nullptr,0,nullptr,1,&b);vkEndCommandBuffer(cmd);VkSubmitInfo sub{VK_STRUCTURE_TYPE_SUBMIT_INFO};sub.commandBufferCount=1;sub.pCommandBuffers=&cmd;auto result=vkQueueSubmit(queue_,1,&sub,VK_NULL_HANDLE);if(result==VK_SUCCESS)result=vkQueueWaitIdle(queue_);void*m=nullptr;if(result==VK_SUCCESS&&vkMapMemory(d_,memory,0,bytes,0,&m)==VK_SUCCESS){std::memcpy(out.data(),m,size_t(bytes));vkUnmapMemory(d_,memory);}else result=VK_ERROR_UNKNOWN;vkFreeCommandBuffers(d_,pool_,1,&cmd);vkDestroyBuffer(d_,buffer,nullptr);vkFreeMemory(d_,memory,nullptr);return result==VK_SUCCESS?DIGITOR_RESULT_OK:DIGITOR_RESULT_BACKEND_UNAVAILABLE;
  }
  DigitorResult execute_process_curves_gpu(std::span<const Color>src,uint32_t width,uint32_t height,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{if(gpu_failure_point()!=GpuFailurePoint::None&&gpu_failure_point()!=GpuFailurePoint::ShaderCompilation&&gpu_failure_point()!=GpuFailurePoint::PipelineCreation)return injected_failure(gpu_failure_point());
    ...
    auto owner=std::shared_ptr<VkPreviewOwner>(new(std::nothrow)VkPreviewOwner{});if(!owner)return DIGITOR_RESULT_OUT_OF_MEMORY;owner->device=d_;owner->device_live=device_live_;
    ...
    auto cached=color_pipeline(true,binary);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;VkDescriptorSetLayout sl=cached->descriptors;VkPipelineLayout pl=cached->layout;VkPipeline pipe=cached->pipeline;VkDescriptorPool dp{};VkCommandBuffer cmd{};auto finish=[&]{if(cmd)vkFreeCommandBuffers(d_,pool_,1,&cmd);if(dp)vkDestroyDescriptorPool(d_,dp,nullptr);cleanup();};
  ...
  DigitorResult execute_process_curves_gpu(const GpuSourceResource&s,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{if(gpu_failure_point()!=GpuFailurePoint::None&&gpu_failure_point()!=GpuFailurePoint::ShaderCompilation&&gpu_failure_point()!=GpuFailurePoint::PipelineCreation)return injected_failure(gpu_failure_point());
    out.reset();auto prior=std::static_pointer_cast<VkPreviewOwner>(native_owner(*s.frame));if(!prior||prior->device!=d_||!prior->output||!prior->output_view||prior->output_layout!=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)return DIGITOR_RESULT_INVALID_ARGUMENT;const auto width=s.width,height=s.height;
    ShaderCompileRequest request{.source=digitor_rgb_curves_hlsl,.entry_point="main",.source_name="rgb_curves.hlsl",.target_profile="cs_6_0",.stage=ShaderStage::compute,.backend=ShaderBackend::vulkan,.macros={{"DIGITOR_VULKAN","1"},{"DIGITOR_TEXTURE_OUTPUT","1"}},.include_roots={},.specialization_constants={},.optimization=ShaderOptimization::performance,.debug_info=false};auto binary=shader_cache_.get_or_compile(shader_compiler_,request);if(!binary)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto owner=std::shared_ptr<VkPreviewOwner>(new(std::nothrow)VkPreviewOwner{});if(!owner)return DIGITOR_RESULT_OUT_OF_MEMORY;owner->device=d_;owner->device_live=device_live_;owner->upstream=prior;
    auto image=[&](VkImageUsageFlags usage,VkImage&i,VkDeviceMemory&m){VkImageCreateInfo c{VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};c.imageType=VK_IMAGE_TYPE_2D;c.extent={width,height,1};c.mipLevels=c.arrayLayers=1;c.format=VK_FORMAT_R32G32B32A32_SFLOAT;c.tiling=VK_IMAGE_TILING_OPTIMAL;c.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;c.samples=VK_SAMPLE_COUNT_1_BIT;c.sharingMode=VK_SHARING_MODE_EXCLUSIVE;c.usage=usage;if(vkCreateImage(d_,&c,nullptr,&i)!=VK_SUCCESS)return false;VkMemoryRequirements r{};vkGetImageMemoryRequirements(d_,i,&r);auto mt=mem(r.memoryTypeBits,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);VkMemoryAllocateInfo a{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};a.allocationSize=r.size;a.memoryTypeIndex=mt;return mt!=UINT32_MAX&&vkAllocateMemory(d_,&a,nullptr,&m)==VK_SUCCESS&&vkBindImageMemory(d_,i,m,0)==VK_SUCCESS;};
    if(!image(VK_IMAGE_USAGE_STORAGE_BIT|VK_IMAGE_USAGE_TRANSFER_SRC_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->output,owner->output_memory)||!image(VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT,owner->preview,owner->preview_memory))return DIGITOR_RESULT_OUT_OF_MEMORY;
    auto view=[&](VkImage i,VkImageView&v){VkImageViewCreateInfo c{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};c.image=i;c.viewType=VK_IMAGE_VIEW_TYPE_2D;c.format=VK_FORMAT_R32G32B32A32_SFLOAT;c.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};return vkCreateImageView(d_,&c,nullptr,&v)==VK_SUCCESS;};if(!view(owner->output,owner->output_view))return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    auto lut=native_rgb_curves_lut(curves);auto params=native_rgb_curves_parameters(curves,width*height);params.padding[0]=width;params.padding[1]=height;VkBuffer b[3]{};VkDeviceMemory bm[3]{};VkDeviceSize sizes[]{0,lut.size()*sizeof(float),sizeof(params)};auto cleanup=[&]{for(int n=0;n<3;n++){if(b[n])vkDestroyBuffer(d_,b[n],nullptr);if(bm[n])vkFreeMemory(d_,bm[n],nullptr);}};
    for(int n=1;n<3;n++){VkBufferCreateInfo c{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};c.size=sizes[n];c.usage=n?VK_BUFFER_USAGE_STORAGE_BUFFER_BIT:VK_BUFFER_USAGE_TRANSFER_SRC_BIT;if(n==2)c.usage=VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;c.sharingMode=VK_SHARING_MODE_EXCLUSIVE;if(vkCreateBuffer(d_,&c,nullptr,&b[n])!=VK_SUCCESS){cleanup();return DIGITOR_RESULT_OUT_OF_MEMORY;}VkMemoryRequirements r{};vkGetBufferMemoryRequirements(d_,b[n],&r);auto mt=mem(r.memoryTypeBits,VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);VkMemoryAllocateInfo a{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};a.allocationSize=r.size;a.memoryTypeIndex=mt;if(mt==UINT32_MAX||vkAllocateMemory(d_,&a,nullptr,&bm[n])!=VK_SUCCESS||vkBindBufferMemory(d_,b[n],bm[n],0)!=VK_SUCCESS){cleanup();return DIGITOR_RESULT_OUT_OF_MEMORY;}}
    void*m=nullptr;for(int n=1;n<3;n++){vkMapMemory(d_,bm[n],0,sizes[n],0,&m);std::memcpy(m,n==1?static_cast<const void*>(lut.data()):static_cast<const void*>(&params),sizes[n]);vkUnmapMemory(d_,bm[n]);}
    auto cached=color_pipeline(true,binary);if(!cached)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;VkDescriptorSetLayout sl=cached->descriptors;VkPipelineLayout pl=cached->layout;VkPipeline pipe=cached->pipeline;VkDescriptorPool dp{};VkCommandBuffer cmd{};auto finish=[&]{if(cmd)vkFreeCommandBuffers(d_,pool_,1,&cmd);if(dp)vkDestroyDescriptorPool(d_,dp,nullptr);cleanup();};
    VkDescriptorPoolSize ps[]{{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1},{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1},{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,1},{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1}};VkDescriptorPoolCreateInfo di{VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO};di.maxSets=1;di.poolSizeCount=4;di.pPoolSizes=ps;if(vkCreateDescriptorPool(d_,&di,nullptr,&dp)!=VK_SUCCESS){finish();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorSetAllocateInfo ai{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};ai.descriptorPool=dp;ai.descriptorSetCount=1;ai.pSetLayouts=&sl;VkDescriptorSet set{};if(vkAllocateDescriptorSets(d_,&ai,&set)!=VK_SUCCESS){finish();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorImageInfo ii[2]{{VK_NULL_HANDLE,prior->output_view,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},{VK_NULL_HANDLE,owner->output_view,VK_IMAGE_LAYOUT_GENERAL}};VkDescriptorBufferInfo bi[2]{{b[1],0,sizes[1]},{b[2],0,sizes[2]}};VkWriteDescriptorSet w[4]{};for(int n=0;n<4;n++){w[n]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};w[n].dstSet=set;w[n].dstBinding=n;w[n].descriptorCount=1;}w[0].descriptorType=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;w[0].pImageInfo=&ii[0];w[1].descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;w[1].pImageInfo=&ii[1];w[2].descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;w[2].pBufferInfo=&bi[0];w[3].descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w[3].pBufferInfo=&bi[1];vkUpdateDescriptorSets(d_,4,w,0,nullptr);
    VkCommandBufferAllocateInfo ca{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};ca.commandPool=pool_;ca.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ca.commandBufferCount=1;if(vkAllocateCommandBuffers(d_,&ca,&cmd)!=VK_SUCCESS){finish();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkCommandBufferBeginInfo cb{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};vkBeginCommandBuffer(cmd,&cb);auto barrier=[&](VkImage image,VkImageLayout old,VkImageLayout now,VkAccessFlags srca,VkAccessFlags dsta,VkPipelineStageFlags srcs,VkPipelineStageFlags dsts){VkImageMemoryBarrier x{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};x.oldLayout=old;x.newLayout=now;x.srcAccessMask=srca;x.dstAccessMask=dsta;x.image=image;x.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};vkCmdPipelineBarrier(cmd,srcs,dsts,0,0,nullptr,0,nullptr,1,&x);};barrier(prior->output,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,VK_ACCESS_SHADER_WRITE_BIT,VK_ACCESS_SHADER_READ_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);barrier(owner->output,VK_IMAGE_LAYOUT_UNDEFINED,VK_IMAGE_LAYOUT_GENERAL,0,VK_ACCESS_SHADER_WRITE_BIT,VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT);vkCmdBindPipeline(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,pipe);vkCmdBindDescriptorSets(cmd,VK_PIPELINE_BIND_POINT_COMPUTE,pl,0,1,&set,0,nullptr);vkCmdDispatch(cmd,(width*height+63)/64,1,1);barrier(owner->output,VK_IMAGE_LAYOUT_GENERAL,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,VK_ACCESS_SHADER_WRITE_BIT,VK_ACCESS_SHADER_READ_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT|VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);vkEndCommandBuffer(cmd);VkSubmitInfo sub{VK_STRUCTURE_TYPE_SUBMIT_INFO};sub.commandBufferCount=1;sub.pCommandBuffers=&cmd;auto vr=vkQueueSubmit(queue_,1,&sub,VK_NULL_HANDLE);if(vr==VK_SUCCESS)vr=vkQueueWaitIdle(queue_);finish();if(vr!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;
    static std::atomic_uint64_t ids{300000};out=std::make_shared<ProcessedGpuFrame>(this,DIGITOR_RENDERER_VULKAN,GpuFrameMetadata{width,height,s.format,GpuFrameAlpha::straight,timestamp,s.color_metadata_identity},ids++,std::static_pointer_cast<void>(owner),std::make_shared<std::atomic_bool>(true),true);provenance_.curve_source_bound=provenance_.curve_destination_bound=provenance_.curve_lut_bound=provenance_.curve_parameters_bound=provenance_.command_recorded=provenance_.dispatch_or_draw_issued=provenance_.queue_submission_issued=provenance_.synchronization_waited=provenance_.output_written=true;provenance_.readback_performed=false;return DIGITOR_RESULT_OK;
  }
  DigitorResult execute_create_preview_consumer(const ProcessedGpuFramePtr&frame,std::shared_ptr<PreviewConsumerDestination>&out)noexcept override{if(gpu_failure_point()==GpuFailurePoint::PreviewAcquisition)return injected_failure(GpuFailurePoint::PreviewAcquisition);
    out.reset();if(!frame||!d_||!device_live_->load())return DIGITOR_RESULT_NOT_INITIALIZED;const auto&m=frame->metadata();auto owner=std::make_shared<VkConsumerOwner>();owner->device=d_;owner->device_live=device_live_;VkImageCreateInfo c{VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};c.imageType=VK_IMAGE_TYPE_2D;c.extent={m.width,m.height,1};c.mipLevels=c.arrayLayers=1;c.format=VK_FORMAT_R32G32B32A32_SFLOAT;c.tiling=VK_IMAGE_TILING_OPTIMAL;c.initialLayout=VK_IMAGE_LAYOUT_UNDEFINED;c.samples=VK_SAMPLE_COUNT_1_BIT;c.sharingMode=VK_SHARING_MODE_EXCLUSIVE;c.usage=VK_IMAGE_USAGE_TRANSFER_DST_BIT|VK_IMAGE_USAGE_SAMPLED_BIT;if(vkCreateImage(d_,&c,nullptr,&owner->image)!=VK_SUCCESS)return DIGITOR_RESULT_OUT_OF_MEMORY;VkMemoryRequirements r{};vkGetImageMemoryRequirements(d_,owner->image,&r);auto mt=mem(r.memoryTypeBits,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);VkMemoryAllocateInfo a{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};a.allocationSize=r.size;a.memoryTypeIndex=mt;if(mt==UINT32_MAX||vkAllocateMemory(d_,&a,nullptr,&owner->memory)!=VK_SUCCESS||vkBindImageMemory(d_,owner->image,owner->memory,0)!=VK_SUCCESS)return DIGITOR_RESULT_OUT_OF_MEMORY;VkImageViewCreateInfo vi{VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO};vi.image=owner->image;vi.viewType=VK_IMAGE_VIEW_TYPE_2D;vi.format=VK_FORMAT_R32G32B32A32_SFLOAT;vi.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};if(vkCreateImageView(d_,&vi,nullptr,&owner->view)!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;auto live=device_live_;auto consumer_live=std::make_shared<std::atomic_bool>(true);static std::atomic_uint64_t tokens{1};out=std::make_shared<PreviewConsumerDestination>(PreviewConsumerMetadata{DIGITOR_RENDERER_VULKAN,this,m.width,m.height,m.format,GpuPrecisionMode::Float32},tokens++,std::static_pointer_cast<void>(owner),consumer_live,[this,live](const ProcessedGpuFramePtr&f,const std::shared_ptr<void>&d){if(gpu_failure_point()==GpuFailurePoint::PreviewPresentation)return injected_failure(GpuFailurePoint::PreviewPresentation);if(!live->load())return DIGITOR_RESULT_NOT_INITIALIZED;auto source=std::static_pointer_cast<VkPreviewOwner>(native_owner(*f));auto destination=std::static_pointer_cast<VkConsumerOwner>(d);if(!source||!destination||source->device!=d_||destination->device!=d_)return DIGITOR_RESULT_INVALID_ARGUMENT;VkCommandBuffer cmd{};VkCommandBufferAllocateInfo ca{VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};ca.commandPool=pool_;ca.level=VK_COMMAND_BUFFER_LEVEL_PRIMARY;ca.commandBufferCount=1;if(vkAllocateCommandBuffers(d_,&ca,&cmd)!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;VkCommandBufferBeginInfo bi{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};vkBeginCommandBuffer(cmd,&bi);VkImageMemoryBarrier bars[2]{};for(auto&b:bars){b={VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER};b.subresourceRange={VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1};}bars[0].image=source->output;bars[0].oldLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;bars[0].newLayout=VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;bars[0].srcAccessMask=VK_ACCESS_SHADER_READ_BIT;bars[0].dstAccessMask=VK_ACCESS_TRANSFER_READ_BIT;bars[1].image=destination->image;bars[1].oldLayout=VK_IMAGE_LAYOUT_UNDEFINED;bars[1].newLayout=VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;bars[1].dstAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT;vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT|VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,0,0,nullptr,0,nullptr,2,bars);VkImageCopy copy{};copy.srcSubresource={VK_IMAGE_ASPECT_COLOR_BIT,0,0,1};copy.dstSubresource=copy.srcSubresource;copy.extent={f->metadata().width,f->metadata().height,1};vkCmdCopyImage(cmd,source->output,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,destination->image,VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,1,&copy);bars[0].oldLayout=VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;bars[0].newLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;bars[0].srcAccessMask=VK_ACCESS_TRANSFER_READ_BIT;bars[0].dstAccessMask=VK_ACCESS_SHADER_READ_BIT;bars[1].oldLayout=VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;bars[1].newLayout=VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;bars[1].srcAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT;bars[1].dstAccessMask=VK_ACCESS_SHADER_READ_BIT;vkCmdPipelineBarrier(cmd,VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT|VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,0,0,nullptr,0,nullptr,2,bars);auto result=vkEndCommandBuffer(cmd);if(result==VK_SUCCESS){VkSubmitInfo sub{VK_STRUCTURE_TYPE_SUBMIT_INFO};sub.commandBufferCount=1;sub.pCommandBuffers=&cmd;result=vkQueueSubmit(queue_,1,&sub,VK_NULL_HANDLE);if(result==VK_SUCCESS)result=vkQueueWaitIdle(queue_);}vkFreeCommandBuffers(d_,pool_,1,&cmd);return result==VK_SUCCESS?DIGITOR_RESULT_OK:DIGITOR_RESULT_BACKEND_UNAVAILABLE;});return DIGITOR_RESULT_OK;
  }
