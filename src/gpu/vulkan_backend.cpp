...
public:
  ...
  DigitorResult execute_process_primary_wheels_gpu(const GpuSourceResource&s,int64_t timestamp,const PrimaryWheelsParameters&parameters,ProcessedGpuFramePtr&out)noexcept override{
    ...
    VkDescriptorSetLayoutBinding bindings[3]{{0,VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr},{1,VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr},{2,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1,VK_SHADER_STAGE_COMPUTE_BIT,nullptr}};VkDescriptorSetLayout sl{};VkPipelineLayout pl{};VkShaderModule sm{};VkPipeline pipe{};VkDescriptorPool dp{};VkCommandBuffer cmd{};auto done=[&]{if(cmd)vkFreeCommandBuffers(d_,pool_,1,&cmd);if(dp)vkDestroyDescriptorPool(d_,dp,nullptr);if(pipe)vkDestroyPipeline(d_,pipe,nullptr);if(sm)vkDestroyShaderModule(d_,sm,nullptr);if(pl)vkDestroyPipelineLayout(d_,pl,nullptr);if(sl)vkDestroyDescriptorSetLayout(d_,sl,nullptr);if(pb)vkDestroyBuffer(d_,pb,nullptr);if(pm)vkFreeMemory(d_,pm,nullptr);};VkDescriptorSetLayoutCreateInfo si{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO};si.bindingCount=3;si.pBindings=bindings;if(vkCreateDescriptorSetLayout(d_,&si,nullptr,&sl)!=VK_SUCCESS)return DIGITOR_RESULT_BACKEND_UNAVAILABLE;VkPipelineLayoutCreateInfo pi{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};pi.setLayoutCount=1;pi.pSetLayouts=&sl;if(vkCreatePipelineLayout(d_,&pi,nullptr,&pl)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkShaderModuleCreateInfo mi{VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};mi.codeSize=binary.binary.size();mi.pCode=reinterpret_cast<const uint32_t*>(binary.binary.data());if(vkCreateShaderModule(d_,&mi,nullptr,&sm)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkComputePipelineCreateInfo ci{VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO};ci.stage={VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO};ci.stage.stage=VK_SHADER_STAGE_COMPUTE_BIT;ci.stage.module=sm;ci.stage.pName="main";ci.layout=pl;if(vkCreateComputePipelines(d_,VK_NULL_HANDLE,1,&ci,nullptr,&pipe)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorPoolSize ps[]{{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,1},{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,1},{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,1}};VkDescriptorPoolCreateInfo di{VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO};di.maxSets=1;di.poolSizeCount=3;di.pPoolSizes=ps;if(vkCreateDescriptorPool(d_,&di,nullptr,&dp)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorSetAllocateInfo ai{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};ai.descriptorPool=dp;ai.descriptorSetCount=1;ai.pSetLayouts=&sl;VkDescriptorSet set{};if(vkAllocateDescriptorSets(d_,&ai,&set)!=VK_SUCCESS){done();return DIGITOR_RESULT_BACKEND_UNAVAILABLE;}VkDescriptorImageInfo ii[2]{{VK_NULL_HANDLE,prior->output_view,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},{VK_NULL_HANDLE,owner->output_view,VK_IMAGE_LAYOUT_GENERAL}};VkDescriptorBufferInfo bi{pb,0,sizeof(params)};VkWriteDescriptorSet w[3]{};for(int n=0;n<3;n++){w[n]={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};w[n].dstSet=set;w[n].dstBinding=n;w[n].descriptorCount=1;}w[0].descriptorType=VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;w[0].pImageInfo=&ii[0];w[1].descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;w[1].pImageInfo=&ii[1];w[2].descriptorType=VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;w[2].pBufferInfo=&bi;vkUpdateDescriptorSets(d_,3,w,0,nullptr);
  ...
  DigitorResult execute_process_curves_gpu(const GpuSourceResource&s,int64_t timestamp,const CompiledRgbCurves&curves,ProcessedGpuFramePtr&out)noexcept override{
