...
NodeId ProductionNodeGraph::convert_to_parallel(NodeId existing,std::string new_branch){
 auto current=node(existing);if(current.kind!=ProductionNodeKind::serial)throw std::invalid_argument("only serial node can convert to parallel");
 if(current.inputs.size()!=1)throw std::invalid_argument("serial node must have one input");
 const auto upstream=current.inputs.front();auto downstream=consumers(existing);
 nodes_.at(existing).kind=ProductionNodeKind::parallel;nodes_.at(existing).position.y-=90;
 NodeId branch=next_++,mix=next_++;auto base=nodes_.at(existing).position;
 nodes_[branch]={branch,ProductionNodeKind::parallel,std::move(new_branch),{upstream},{},true,false,{base.x,base.y+180}};
 nodes_[mix]={mix,ProductionNodeKind::mixer,"Parallel Mixer",{existing,branch},{},true,false,{base.x+160,base.y+90}};
 for(auto d:downstream){if(d==mix)continue;for(auto&i:nodes_.at(d).inputs)if(i==existing)i=mix;}
 selected_=branch;return branch;
}

void ProductionNodeGraph::normalize_mixers(){
 bool changed=true;while(changed){changed=false;std::vector<NodeId> mids;for(auto&[id,n]:nodes_)if(n.kind==ProductionNodeKind::mixer)mids.push_back(id);
 for(auto mid:mids){if(!nodes_.contains(mid))continue;auto inputs=nodes_.at(mid).inputs;inputs.erase(std::remove_if(inputs.begin(),inputs.end(),[&](NodeId x){return !nodes_.contains(x);}),inputs.end());nodes_.at(mid).inputs=inputs;
 if(inputs.size()>=2){continue;}NodeId replacement=inputs.empty()?input_:inputs.front();for(auto&[nid,n]:nodes_)if(nid!=mid)for(auto&i:n.inputs)if(i==mid)i=replacement;nodes_.erase(mid);if(nodes_.contains(replacement)&&nodes_.at(replacement).kind==ProductionNodeKind::parallel)nodes_.at(replacement).kind=ProductionNodeKind::serial;changed=true;break;}}
}
...
void ProductionNodeGraph::trim_render_cache(NodeId protected_id,NodeRenderStats* stats)const{
 while(render_cache_bytes_>render_cache_budget_bytes_&&!render_cache_.empty()){
  auto victim=render_cache_.end();
  for(auto it=render_cache_.begin();it!=render_cache_.end();++it){if(it->first==protected_id)continue;if(victim==render_cache_.end()||it->second.last_use<victim->second.last_use)victim=it;}
  if(victim==render_cache_.end()) break;
  render_cache_bytes_-=victim->second.bytes;
  render_cache_.erase(victim);
  if(stats) ++stats->cache_evictions;
 }
}
NodeValue ProductionNodeGraph::render_cached(std::span<const Color>source,std::uint32_t w,std::uint32_t h,std::uint64_t source_generation,NodeRenderStats* stats,std::int64_t frame)const{
 if(source.size()!=std::size_t(w)*h)throw std::invalid_argument("source dimensions");
 NodeRenderStats local{};std::unordered_map<NodeId,NodeValue>values;std::unordered_map<NodeId,std::string>keys;
 for(auto id:execution_order()){
  const auto&n=node(id);std::ostringstream key;
  if(n.kind==ProductionNodeKind::input){key<<"source:"<<source_generation<<':'<<w<<'x'<<h<<':'<<frame;}
  else{key<<id<<':'<<unsigned(n.kind)<<':'<<n.enabled<<':'<<n.bypassed;for(auto x:n.inputs)key<<"|in="<<keys.at(x);for(const auto&o:n.operations)key<<"|op="<<unsigned(o.kind)<<':'<<o.enabled<<':'<<o.identity;}
  auto key_string=key.str();auto cached=render_cache_.find(id);
  if(cached!=render_cache_.end()&&cached->second.key==key_string){cached->second.last_use=++render_cache_clock_;values[id]=cached->second.value;++local.cache_hits;keys[id]=std::move(key_string);continue;}
  ++local.cache_misses;++local.executed_nodes;NodeValue v;
  if(n.kind==ProductionNodeKind::input)v.assign(source.begin(),source.end());
  else{std::vector<NodeValue>in;for(auto x:n.inputs)in.push_back(values.at(x));v=n.kind==ProductionNodeKind::mixer?mix_inputs(in):(in.empty()?NodeValue{}:in.front());if(n.enabled&&!n.bypassed&&n.kind!=ProductionNodeKind::output)v=apply_operations(n,std::move(v),w,h);}
  if(cached!=render_cache_.end()){render_cache_bytes_-=cached->second.bytes;render_cache_.erase(cached);}
  const auto entry_bytes=v.size()*sizeof(Color)+key_string.size();render_cache_[id]={key_string,v,++render_cache_clock_,entry_bytes};render_cache_bytes_+=entry_bytes;trim_render_cache(id,&local);values[id]=std::move(v);keys[id]=std::move(key_string);
 }
 local.cache_bytes=render_cache_bytes_;if(stats)*stats=local;
 return values.at(output_);
}
