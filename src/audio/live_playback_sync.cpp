...
LivePlaybackSync::LivePlaybackSync(std::int64_t manual_offset_us,
  ...
  compensation_ = make_audio_sync_compensation({}, manual_offset_us_, manual_override_);
}
...
void LivePlaybackSync::set_manual_offset(std::int64_t offset_us,
  ...
  compensation_ = make_audio_sync_compensation(probe, manual_offset_us_, manual_override_);
}
...
std::size_t LivePlaybackSync::select_frame(
  ...
  for (std::size_t i = 0; i < frames.size(); ++i) {
    ...
    if (clock < frames[i].pts_us + duration) break;
    selected = i;
  }
  return selected;
}
...
PlaybackSyncSnapshot LivePlaybackSync::snapshot() const noexcept {
  return {compensation_, probe_generation_, last_probe_time_us_, device_change_pending_};
}
