...
[[nodiscard]] bool valid_format(const AndroidHardwareBufferFrame& f) noexcept {
  if (!f.hardware_buffer || !f.width || !f.height || f.timestamp_us < 0 ||
      f.frame_identity == 0) return false;
...
DigitorResult AndroidMediaCodecSurfaceDecoder::acquire(
  ...
  void* buffer{}; int fence_fd{-1}; std::shared_ptr<void> lifetime;
...
AndroidMediaCodecAcquire AndroidMediaCodecSurfaceDecoder::callback() {
  ...
  return [keep](std::int64_t t, AndroidHardwareBufferFrame& f) noexcept {
    ...
    d.impl_ = keep; return d.acquire(t, f);
  };
}
...
DigitorResult AndroidVulkanHardwareBufferImporter::initialize() noexcept {
  if (!impl_->config.instance || !impl_->config.physical_device || !impl_->config.device ||
      !impl_->config.queue) return DIGITOR_RESULT_INVALID_ARGUMENT;
#ifndef __ANDROID__
  return DIGITOR_RESULT_UNSUPPORTED;
#else
  impl_->initialized = true;
...
DigitorResult AndroidVulkanHardwareBufferImporter::import(
  ...
  // Native VkImage/VkDeviceMemory/VkImageView creation is delegated to the
  // application Vulkan owner so the engine never mixes devices or queues.
  // The returned wrapper must own AHardwareBuffer_acquire/release and imported
  // fence-fd lifetime. A null native object is rejected fail-closed.
...
AndroidVulkanImport AndroidVulkanHardwareBufferImporter::callback() {
  ...
  return [keep](const AndroidHardwareBufferFrame& f, AndroidImportedImage& i) noexcept {
    AndroidVulkanHardwareBufferImporter v(keep->config); v.impl_ = keep;
    return v.import(f, i);
  };
}
...
AndroidGlesHardwareBufferImporter::AndroidGlesHardwareBufferImporter(
    AndroidGlesExternalImageConfig c) : impl_(std::make_shared<Impl>()) { impl_->config=c; }
...
AndroidGlesImport AndroidGlesHardwareBufferImporter::callback() {
  auto keep=impl_;
  return [keep](const AndroidHardwareBufferFrame& f, AndroidImportedImage& i) noexcept {
    AndroidGlesHardwareBufferImporter g(keep->config); g.impl_=keep; return g.import(f,i);
  };
}
...
AndroidYuvToRgba16f AndroidGpuYuvConverter::callback() {
  return [dispatch=dispatch_](const AndroidImportedImage& in, ProcessedGpuFramePtr& out) noexcept {
    AndroidGpuYuvConverter c(dispatch); return c.convert(in,out);
  };
}
...
AndroidMediaCodecHardwareEncoder::AndroidMediaCodecHardwareEncoder(
    ...
    : impl_(std::make_shared<Impl>()) { impl_->config=std::move(c);impl_->submit=std::move(s); }
...
DigitorResult AndroidMediaCodecHardwareEncoder::initialize() noexcept {
  if (impl_->config.mime.empty() || !impl_->config.width || !impl_->config.height ||
      !impl_->config.fps || !impl_->config.bitrate || !impl_->config.input_surface || !impl_->submit)
    return DIGITOR_RESULT_INVALID_ARGUMENT;
  if (impl_->config.bit_depth != 8 && impl_->config.bit_depth != 10)
    return DIGITOR_RESULT_UNSUPPORTED;
#ifndef __ANDROID__
  return DIGITOR_RESULT_UNSUPPORTED;
#else
  impl_->initialized=true; return DIGITOR_RESULT_OK;
#endif
}
...
AndroidGpuFrameConsumer AndroidMediaCodecHardwareEncoder::callback(){
  ...
  return [keep](const ProcessedGpuFramePtr& f) noexcept {
    AndroidMediaCodecHardwareEncoder e(keep->config,keep->submit);e.impl_=keep;return e.consume(f);
  };
}
...
AndroidConcreteZeroCopyPipeline::AndroidConcreteZeroCopyPipeline(
  ...
  impl_->decoder=std::make_unique<AndroidMediaCodecSurfaceDecoder>(b.decoder,b.decoder_surface_acquire);
  impl_->vulkan=std::make_unique<AndroidVulkanHardwareBufferImporter>(b.vulkan);
  impl_->gles=std::make_unique<AndroidGlesHardwareBufferImporter>(b.gles);
...
DigitorResult AndroidConcreteZeroCopyPipeline::initialize() noexcept {
  auto r=impl_->decoder->initialize();if(r!=DIGITOR_RESULT_OK)return r;
  r=impl_->vulkan->initialize();
  if(r!=DIGITOR_RESULT_OK){auto g=impl_->gles->initialize();if(g!=DIGITOR_RESULT_OK)return r;}
  r=impl_->encoder->initialize();if(r!=DIGITOR_RESULT_OK)return r;
  return impl_->pipeline->initialize();
}
DigitorResult AndroidConcreteZeroCopyPipeline::preview(std::int64_t t) noexcept{return impl_->pipeline->preview(t);}
DigitorResult AndroidConcreteZeroCopyPipeline::export_frame(std::int64_t t) noexcept{return impl_->pipeline->export_frame(t);}
DigitorResult AndroidConcreteZeroCopyPipeline::preview_and_export(std::int64_t t) noexcept{return impl_->pipeline->preview_and_export(t);}
AndroidZeroCopyTelemetry AndroidConcreteZeroCopyPipeline::telemetry() const{return impl_->pipeline->telemetry();}
