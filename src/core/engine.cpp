...
DigitorResult Engine::process_log_wheels_gpu(std::span<const Color>s,std::uint32_t w,std::uint32_t h,std::int64_t ts,const LogWheelsParameters&p,ProcessedGpuFramePtr&out){std::lock_guard lock(mutex_);out.reset();if(!initialized_||!backend_)return DIGITOR_RESULT_NOT_INITIALIZED;return backend_->process_log_wheels_gpu(s,w,h,ts,p,out);}
DigitorResult Engine::process_log_wheels_gpu(const ProcessedGpuFramePtr&s,std::int64_t ts,const LogWheelsParameters&p,ProcessedGpuFramePtr&out){std::lock_guard lock(mutex_);out.reset();if(!initialized_||!backend_)return DIGITOR_RESULT_NOT_INITIALIZED;return backend_->process_log_wheels_gpu(backend_->gpu_source(s),ts,p,out);}
