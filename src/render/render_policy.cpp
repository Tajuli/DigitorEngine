...
const MediaSourceDescriptor* find_source(std::span<const MediaSourceDescriptor> sources,
  ...
  for (const auto& source : sources)
    if (source.source_class == source_class) return &source;
  return nullptr;
}
...
std::vector<std::string> ColorGraphConfiguration::operation_sequence() const {
  ...
  auto add_wheels = [&] { if (primary_wheels_enabled) result.emplace_back("primary-wheels-v1"); };
  auto add_curves = [&] { if (rgb_curves_enabled) result.emplace_back("rgb-curves-v1"); };
  if (operation_order == ColorOperationOrder::PrimaryWheelsThenRgbCurves) {
    add_wheels(); add_curves();
  } else {
    add_curves(); add_wheels();
  }
  return result;
}
...
std::string ColorGraphConfiguration::identity() const {
  ...
  number(result, primary_wheels_enabled);
  field(result, primary_wheels_serialization);
  number(result, rgb_curves_enabled);
  field(result, rgb_curves_serialization);
...
ColorRenderPlan build_color_render_plan(RenderPurpose purpose,
  ...
  const auto wanted = purpose == RenderPurpose::Export ? MediaSourceClass::Original
                                                       : preview.requested_class;
  const auto* selected = find_source(sources, wanted);
  if (!selected)
    throw std::invalid_argument(purpose == RenderPurpose::Export
        ? "export requires an original full-quality source"
        : "requested preview source is unavailable");
  validate(*selected);
  ...
  const auto& media_identity = original ? original->original_media_identity
                                        : sources.front().original_media_identity;
  ...
  field(key, selected->source_identity);
  field(key, selected->original_media_identity);
  number(key, selected->width); number(key, selected->height);
  number(key, static_cast<std::uint32_t>(selected->precision));
  field(key, selected->color_metadata_identity);
  ...
  if (purpose == RenderPurpose::Preview && preview.frame_rate_policy) {
    number(key, preview.frame_rate_policy->numerator);
    number(key, preview.frame_rate_policy->denominator);
