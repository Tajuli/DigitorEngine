...
bool qualify_numerical(IRenderBackend& backend, std::string_view name) {
  ...
  const bool passed =
      ...
      provenance && metrics.failures == 0 && metrics.rms <= 5.0e-6 &&
      metrics.ssim >= 0.99999;
...
bool qualify_gpu_source_chain(IRenderBackend& backend, std::string_view name) {
  ...
  const bool passed =
      ...
      metrics.failures == 0 && metrics.rms <= 5.0e-6 &&
      metrics.ssim >= 0.99999;
...
std::vector<GpuFailurePoint> process_failure_points(
  ...
  switch (backend) {
    case DIGITOR_RENDERER_VULKAN:
      return {F::ShaderCompilation, F::DescriptorSetLayoutCreation,
              F::PipelineLayoutCreation, F::PipelineCreation,
              F::SourceResourceCreation, F::SourceMemoryAllocation,
              F::SourceMemoryBinding, F::SourceUpload,
              F::OutputResourceCreation, F::OutputMemoryAllocation,
              F::OutputMemoryBinding, F::ImageViewCreation,
              F::ParameterResourceCreation, F::ParameterUpload,
              F::DescriptorPoolCreation, F::DescriptorSetAllocation,
              F::DescriptorUpdate, F::CommandBufferOrListAllocation,
              F::CommandBufferOrListBeginReset, F::CommandRecording,
              F::DispatchOrDraw, F::CommandBufferOrListClose,
              F::QueueSubmission, F::SynchronizationWait,
              F::ProcessedFrameCreation, F::DeterministicOutOfMemory};
    case DIGITOR_RENDERER_D3D12:
      return {F::ShaderCompilation, F::RootSignatureSerialization,
              F::RootSignatureCreation, F::PipelineCreation,
              F::SourceResourceCreation, F::SourceUpload,
              F::CpuSourceShaderResourceViewCreation,
              F::OutputResourceCreation, F::DescriptorHeapCreation,
              F::UnorderedAccessViewCreation, F::ParameterUpload,
              F::CommandAllocatorResetOrCreation,
              F::CommandBufferOrListBeginReset, F::CommandRecording,
              F::DispatchOrDraw, F::CommandBufferOrListClose,
              F::QueueSubmission, F::FenceSignal, F::EventSetup,
              F::SynchronizationWait, F::SynchronizationVerification,
              F::ProcessedFrameCreation, F::DeterministicOutOfMemory};
    case DIGITOR_RENDERER_METAL:
      return {F::LibraryCreation, F::ShaderFunctionLookup, F::PipelineCreation,
              F::SourceResourceCreation, F::SourceUpload,
              F::OutputResourceCreation, F::ParameterResourceCreation,
              F::CommandQueueCreation, F::CommandBufferOrListAllocation,
              F::ComputeEncoderCreation, F::SourceTextureBinding,
              F::OutputTextureBinding, F::BufferBinding, F::DispatchSetup,
              F::DispatchOrDraw, F::EncoderCompletion, F::QueueSubmission,
              F::SynchronizationWait, F::CommandStatusVerification,
              F::ProcessedFrameCreation, F::DeterministicOutOfMemory};
    case DIGITOR_RENDERER_OPENGL_ES:
      return {F::VertexShaderCreation, F::VertexShaderCompilation,
              F::FragmentShaderCreation, F::FragmentShaderCompilation,
              F::ProgramCreation, F::ProgramLink, F::SourceResourceCreation,
              F::SourceResourceStorage, F::SourceUpload,
              F::OutputResourceCreation, F::OutputResourceStorage,
              F::FramebufferCreation, F::FramebufferAttachment,
              F::FramebufferValidation, F::ResourceBinding, F::UniformLookup,
              F::DrawSetup, F::DispatchOrDraw, F::Flush,
              F::ProcessedFrameCreation, F::DeterministicOutOfMemory};
    default:
      return {};
  }
...
std::vector<GpuFailurePoint> validation_failure_points(
  ...
  switch (backend) {
    case DIGITOR_RENDERER_VULKAN:
      return {F::ValidationReadbackResourceCreation,
              F::BufferMemoryAllocation, F::BufferMemoryBinding,
              F::CommandBufferOrListAllocation,
              F::CommandBufferOrListBeginReset, F::CommandRecording,
              F::ValidationTransition, F::ValidationReadbackCopy,
              F::CommandBufferOrListClose, F::QueueSubmission,
              F::SynchronizationWait, F::ValidationReadbackMap,
              F::DeterministicOutOfMemory};
    case DIGITOR_RENDERER_D3D12:
      return {F::ValidationReadbackResourceCreation,
              F::CommandAllocatorResetOrCreation,
              F::CommandBufferOrListBeginReset, F::ValidationTransition,
              F::ValidationReadbackCopy, F::CommandBufferOrListClose,
              F::QueueSubmission, F::FenceSignal, F::EventSetup,
              F::SynchronizationWait, F::SynchronizationVerification,
    ...
    case DIGITOR_RENDERER_METAL:
      return {F::ValidationReadbackResourceCreation,
              F::CommandBufferOrListAllocation, F::BlitEncoderCreation,
              F::ValidationReadbackCopy, F::EncoderCompletion,
              F::QueueSubmission, F::SynchronizationWait,
              F::CommandStatusVerification, F::ValidationCpuCopy,
              F::DeterministicOutOfMemory};
    case DIGITOR_RENDERER_OPENGL_ES:
      return {F::ValidationReadbackResourceCreation,
              F::ValidationReadbackCopy, F::ValidationReadbackMap};
    default:
      return {};
  }
}

bool is_pipeline_stage(DigitorRendererBackend backend, GpuFailurePoint point) {
  using F = GpuFailurePoint;
  if (backend == DIGITOR_RENDERER_VULKAN)
    return point == F::ShaderCompilation ||
           point == F::DescriptorSetLayoutCreation ||
           point == F::PipelineLayoutCreation ||
           point == F::PipelineCreation;
  if (backend == DIGITOR_RENDERER_D3D12)
    return point == F::ShaderCompilation ||
           point == F::RootSignatureSerialization ||
           point == F::RootSignatureCreation ||
           point == F::PipelineCreation;
  if (backend == DIGITOR_RENDERER_METAL)
    return point == F::LibraryCreation ||
           point == F::ShaderFunctionLookup ||
           point == F::PipelineCreation;
  if (backend == DIGITOR_RENDERER_OPENGL_ES)
    return point == F::VertexShaderCreation ||
           point == F::VertexShaderCompilation ||
           point == F::FragmentShaderCreation ||
           point == F::FragmentShaderCompilation ||
           point == F::ProgramCreation || point == F::ProgramLink;
  return false;
}
...
bool qualify_failure_matrix(IRenderBackend& backend, std::string_view name) {
  ...
  for (const auto point : process_failure_points(backend.info().backend)) {
    ...
    const bool reached =
        evidence.requested_failure_point == point &&
        evidence.actual_stage_reached == point;
    const bool ok =
        failure != DIGITOR_RESULT_OK && !failed && reached &&
        evidence.output_cleared && cleanup && evidence.cache_valid &&
        evidence.cpu_log_wheels_invocations == 0 &&
        evidence.log_wheels_fallback_invocations == 0 &&
        evidence.normal_preview_readback_count == 0 &&
    ...
    std::cerr << "LOG_WHEELS_FAILURE_STAGE backend=" << name
              ...
              << " reached=" << reached
              << " output_cleared=" << evidence.output_cleared
              << " cleanup=" << cleanup
              << " cache_ok=" << evidence.cache_valid
              << " cpu_delta=" << evidence.cpu_log_wheels_invocations
              << " fallback=" << evidence.log_wheels_fallback_invocations
  ...
  ProcessedGpuFramePtr frame;
  digitor::set_gpu_failure_point(GpuFailurePoint::None);
  if (backend.process_log_wheels_gpu(
          input, kWidth, kHeight, 4940, *parameters, frame) != DIGITOR_RESULT_OK ||
      !frame)
    return false;
  ...
  for (const auto point : validation_failure_points(backend.info().backend)) {
    ...
    digitor::set_gpu_failure_point(point);
    const auto failure =
        backend.validation_readback_log_wheels(frame, readback);
    const auto evidence = backend.execution_provenance();
    digitor::set_gpu_failure_point(GpuFailurePoint::None);
    const auto recovery =
        backend.validation_readback_log_wheels(frame, readback);
    const bool reached =
        ...
        evidence.actual_stage_reached == point;
    const bool ok =
        failure != DIGITOR_RESULT_OK && reached &&
        !evidence.validation_readback_completed &&
        evidence.cpu_log_wheels_invocations == 0 &&
        evidence.log_wheels_fallback_invocations == 0 &&
...
bool qualify_cache_failure(IRenderBackend& backend, std::string_view name) {
  const auto input = make_input();
  const auto parameters = make_parameters();
  const auto backend_kind = backend.info().backend;
  GpuFailurePoint point = GpuFailurePoint::PipelineCreation;
  if (backend_kind == DIGITOR_RENDERER_OPENGL_ES)
    point = GpuFailurePoint::ProgramLink;
  ...
  const bool rejected =
      failure != DIGITOR_RESULT_OK && !failed_frame &&
      failed.lookups == before.lookups + 1 &&
      failed.misses == before.misses + 1 &&
      failed.creation_failures == before.creation_failures + 1 &&
      failed.hits == before.hits &&
      backend.native_pipeline_cache_size() == 0 &&
