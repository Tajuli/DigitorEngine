...
bool exercise(digitor::IRenderBackend& backend, std::string_view name) {
    ...
    passed &= grade_result==DIGITOR_RESULT_OK && maximum<tolerance.component_absolute && rms<=tolerance.rms && ssim>=tolerance.ssim;
    ...
    passed &= curve_result==DIGITOR_RESULT_OK && curve_max<tolerance.component_absolute && curve_rms<=tolerance.rms && curve_ssim>=tolerance.ssim &&
