...
void test_color_pipeline(){using namespace digitor;
 ...
 ColorPipelineParameters p;p.primary.exposure=.25f;p.primary.contrast=1.1f;p.red=Curve({{0,0},{.5f,.4f},{1,1}});ColorShaderGraph graph;std::vector<Color> cpu(4),gpu(4);graph.process_cpu(reference,cpu,p);CommandBuffer cb;CommandEncoder enc(cb);bool rejected=false;try{graph.process_gpu(enc,reference,gpu,p);}catch(const std::invalid_argument&){rejected=true;}assert(rejected);assert(graph.schedule().size()==10&&graph.shader_cache_size()==0&&graph.pipeline_cache_size()==0);
 auto metrics=validate_color_pipeline(cpu,cpu,ShaderBackend::vulkan);assert(metrics.passed&&metrics.maximum_absolute_error==0&&metrics.rms==0&&metrics.ssim==1);
 HslQualifier qualifier;QualifierSettings qs;qs.hue={0,1,.1f};qs.saturation={0,1,.1f};qs.luminance={0,1,.1f};qs.blur=1;qualifier.set_settings(qs);auto cm=qualifier.matte_cpu(reference,2,2);assert(cm.size()==4);std::vector<float> gm(4,-1.0f);CommandBuffer qb;CommandEncoder qe(qb);bool qualifier_rejected=false;try{qualifier.matte_gpu(qe,reference,gm,2,2);}catch(const std::logic_error&){qualifier_rejected=true;}assert(qualifier_rejected);for(float value:gm)assert(value==-1.0f);
}
