...
void test_color_pipeline(){using namespace digitor;
 ...
 ColorPipelineParameters p;p.primary.exposure=.25f;p.primary.contrast=1.1f;p.red=Curve({{0,0},{.5f,.4f},{1,1}});ColorShaderGraph graph;std::vector<Color> cpu(4),gpu(4);graph.process_cpu(reference,cpu,p);CommandBuffer cb;CommandEncoder enc(cb);graph.process_gpu(enc,reference,gpu,p);enc.finish();CommandQueue q;q.submit(cb);assert(validate_pixels(cpu,gpu,1e-7f).passed);assert(graph.schedule().size()==4&&graph.shader_cache_size()==1&&graph.pipeline_cache_size()==1);
 HslQualifier qualifier;QualifierSettings qs;qs.hue={0,1,.1f};qs.saturation={0,1,.1f};qs.luminance={0,1,.1f};qs.blur=1;qualifier.set_settings(qs);auto cm=qualifier.matte_cpu(reference,2,2);std::vector<float> gm(4);CommandBuffer qb;CommandEncoder qe(qb);qualifier.matte_gpu(qe,reference,gm,2,2);qe.finish();q.submit(qb);for(size_t n=0;n<4;n++)assert(std::abs(cm[n]-gm[n])<1e-7f);
}
