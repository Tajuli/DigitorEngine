...
int main() {
    ...
    {
        digitor::ShaderCompiler compiler;
        digitor::ShaderCompileRequest request;
        request.source = "[numthreads(8,1,1)] void main(){}";
        const auto result = compiler.compile(request);
        if (result.error == digitor::ShaderError::compiler_unavailable) {
            assert(!result);
            assert(result.binary.empty());
        } else {
            assert(result.error == digitor::ShaderError::none);
            assert(result);
            assert(!result.binary.empty());
            assert(!result.compiler_identity.empty());
        }
    }
