...
void test_editor(){
  // Keep the timeline/audio C ABI linked into the primary DigitorEngine target.
  // The Flutter workspace resolves these symbols from the same native library at
  // runtime, so this test must fail at link time if that ABI is omitted again.
  DigitorTimelineSessionConfig session_config{};
  session_config.sample_rate=48000;session_config.channels=2;session_config.duration_us=1000000;
  DigitorTimelineAudioSession* session=nullptr;
  assert(digitor_timeline_session_create(&session_config,&session)==DIGITOR_RESULT_OK);
