...
struct ExportRenderSnapshotData final {
...
class ExportRenderSnapshot final {
 public:
  explicit ExportRenderSnapshot(ExportRenderSnapshotData data)
      : data_(std::move(data)) {}

  [[nodiscard]] const ExportRenderSnapshotData& data() const noexcept { return data_; }
  [[nodiscard]] std::uint64_t identity() const noexcept { return data_.snapshot_identity; }
  [[nodiscard]] ExportExecutionPolicy policy() const noexcept { return data_.policy; }
  [[nodiscard]] DigitorRendererBackend renderer_backend() const noexcept {
    return data_.renderer_backend;
  }
  [[nodiscard]] EncoderBackend encoder_backend() const noexcept {
    return data_.encoder_backend;
  }

 private:
  const ExportRenderSnapshotData data_;
};
...
[[nodiscard]] inline ExportContractValidation validate_export_snapshot(
    const ExportRenderSnapshot& snapshot) noexcept {
  const auto& value = snapshot.data();
  ...
  if (value.profile.width != static_cast<std::int32_t>(value.width) ||
    ...
    return {DIGITOR_RESULT_INVALID_ARGUMENT,
            "export profile does not match frozen render snapshot"};
  }
  if (value.color_metadata.empty()) {
    return {DIGITOR_RESULT_INVALID_ARGUMENT,
            "export color metadata is required"};
  }
  if (value.policy == ExportExecutionPolicy::hardware_required) {
    if (value.renderer_backend == DIGITOR_RENDERER_CPU) {
      return {DIGITOR_RESULT_UNSUPPORTED,
              "hardware-required export cannot use the CPU renderer"};
    }
    if (value.encoder_backend == EncoderBackend::software) {
      return {DIGITOR_RESULT_UNSUPPORTED,
              "hardware-required export cannot use a software encoder"};
    }
    if (!value.profile.prefer_hardware || value.profile.allow_software_fallback) {
      return {DIGITOR_RESULT_UNSUPPORTED,
              "hardware-required export must disable software fallback before start"};
    }
  } else if (value.policy == ExportExecutionPolicy::explicit_cpu_reference) {
    if (value.renderer_backend != DIGITOR_RENDERER_CPU ||
        value.encoder_backend != EncoderBackend::software ||
        value.profile.prefer_hardware) {
      return {DIGITOR_RESULT_UNSUPPORTED,
              "CPU/reference export must be an explicit all-CPU job"};
    }
  } else {
...
[[nodiscard]] inline ExportContractValidation validate_frame_against_snapshot(
  ...
  if (!frame.ready() || !frame.context_live()) {
    return {DIGITOR_RESULT_BACKEND_UNAVAILABLE,
            "GPU frame is not ready or its context is retired"};
  }
  const auto& value = snapshot.data();
  const auto& metadata = frame.metadata();
  if (metadata.width != value.width || metadata.height != value.height ||
      metadata.format != value.working_format) {
    return {DIGITOR_RESULT_INVALID_ARGUMENT,
            "GPU frame format or dimensions differ from frozen snapshot"};
  }
  if (metadata.color_metadata != value.color_metadata) {
    return {DIGITOR_RESULT_INVALID_ARGUMENT,
            "GPU frame color metadata differs from frozen snapshot"};
  }
  if (value.policy == ExportExecutionPolicy::hardware_required &&
      frame.backend() == DIGITOR_RENDERER_CPU) {
    return {DIGITOR_RESULT_UNSUPPORTED,
            "hardware export rejected a CPU frame"};
  }
  if (value.policy == ExportExecutionPolicy::hardware_required &&
      frame.backend() != value.renderer_backend) {
    return {DIGITOR_RESULT_UNSUPPORTED,
            "GPU frame backend differs from frozen export backend"};
  }
  return {DIGITOR_RESULT_OK, "ok"};
}
