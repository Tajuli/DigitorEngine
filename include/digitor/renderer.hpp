...
class SharedRenderer { public: explicit SharedRenderer(RenderGraphBuilder={}); VideoFrame render(const RenderRequest&); ProcessedGpuFramePtr render_gpu_preview(const RenderRequest&); void set_rgb_curves(std::shared_ptr<const CompiledRgbCurves> curves){curves_=std::move(curves);} void set_primary_wheels(std::shared_ptr<const PrimaryWheelsParameters>p){primary_wheels_=std::move(p);} void set_color_operation_order(ColorOperationOrder o){operation_order_=o;} void set_media_sources(std::vector<MediaSourceDescriptor> sources){media_sources_=std::move(sources);} void set_preview_source_configuration(PreviewSourceConfiguration configuration){preview_source_=std::move(configuration);} [[nodiscard]] ColorRenderPlan color_render_plan(RenderPurpose)const; [[nodiscard]] bool has_media_source_policy()const noexcept{return !media_sources_.empty();} RenderGraph& graph(){return graph_;} std::uint64_t graph_generation()const{return generation_;} private: VideoFrame render_source(const RenderRequest&); RenderGraphBuilder builder_;RenderGraph graph_;CommandQueue queue_;std::shared_ptr<const CompiledRgbCurves> curves_;std::shared_ptr<const PrimaryWheelsParameters> primary_wheels_;ColorOperationOrder operation_order_{ColorOperationOrder::PrimaryWheelsThenRgbCurves};std::vector<MediaSourceDescriptor> media_sources_;PreviewSourceConfiguration preview_source_;std::uint64_t generation_{};};
...
private:
    VideoFrame render_frame(FrameNumber frame, const ExportSettings& settings) {
        if (renderer_.has_media_source_policy())
            (void)renderer_.color_render_plan(RenderPurpose::Export);
