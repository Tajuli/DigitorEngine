...
public:
  ...
  [[nodiscard]] const QualifierSettings& values() const noexcept { return values_; }
...
public:
  ...
  [[nodiscard]] const QualifierSettings& settings() const noexcept { return settings_; }
  void set_settings(QualifierSettings settings);
  [[nodiscard]] std::vector<float> matte_cpu(std::span<const Color>,
      std::uint32_t width, std::uint32_t height) const;

  // Compatibility entry point. A selected GPU backend must use the native
  // IRenderBackend qualifier APIs; CPU callbacks are forbidden here.
