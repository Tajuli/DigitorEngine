...
Future<void> main(List<String> arguments) async {
  await build(arguments, (input, output) async {
    ...
    if (!const <OS>{OS.android, OS.iOS, OS.linux, OS.macOS, OS.windows}.contains(targetOs)) {
      throw UnsupportedError('DigitorEngine does not support ${targetOs.name} native assets.');
    }
    ...
    final libraryUri = outputRoot.resolve(targetOs.dylibFileName('digitor_engine'));

    final ffmpegSdk = await _resolveFfmpegSdk(code, input.userDefines.path('ffmpeg_root'));
    final ffmpegRoot = ffmpegSdk?.root;
    if (ffmpegRoot != null) output.dependencies.add(ffmpegRoot);
    ...
    final configureArguments = <String>[
      '-S', engineRoot.toFilePath(),
      '-B', buildDirectory.toFilePath(),
      ...
      if (ffmpegRoot != null) '-DDIGITOR_FFMPEG_ROOT=${ffmpegRoot.toFilePath()}',
    ...
    await _run(cmake, <String>[
      '--build', buildDirectory.toFilePath(), '--config', 'Release', '--target', 'digitor_engine', '--parallel', '2',
    ], workingDirectory: engineRoot);

    final library = File.fromUri(libraryUri);
    if (!await library.exists()) {
      throw StateError('DigitorEngine build completed but ${library.path} was not produced.');
    }
...
Future<_FfmpegSdk?> _resolveFfmpegSdk(CodeConfig code, Uri? configuredRoot) async {
  if (configuredRoot != null) {
    final directory = Directory.fromUri(configuredRoot);
    if (!await directory.exists()) {
      throw ArgumentError('hooks.user_defines.digitor_engine_ffi.ffmpeg_root does not exist: ${directory.path}');
    }
    return _FfmpegSdk(configuredRoot,
      code.targetOS == OS.windows ? await _windowsRuntimeLibraries(directory) : const <File>[]);
  }
  ...
  final triplet = switch (code.targetArchitecture.name) {
    ...
    'ia32' => 'x86-windows',
    'x64' => 'x64-windows',
    _ => throw UnsupportedError('Unsupported Windows FFmpeg architecture: ${code.targetArchitecture.name}'),
  };

  final profile = _windowsDependencyProfile();
  final cacheRoot = Directory('${profile.localAppData}${Platform.pathSeparator}DigitorEngine${Platform.pathSeparator}deps');
  await cacheRoot.create(recursive: true);

  final existingVcpkg = await _findExistingVcpkg();
  final vcpkgRoot = existingVcpkg?.parent ?? Directory('${cacheRoot.path}${Platform.pathSeparator}vcpkg');
  final vcpkg = existingVcpkg ?? File('${vcpkgRoot.path}${Platform.pathSeparator}vcpkg.exe');

  if (!await vcpkg.exists()) {
    if (await vcpkgRoot.exists()) await vcpkgRoot.delete(recursive: true);
    await _run('git', <String>[
      'clone', '--depth', '1', 'https://github.com/microsoft/vcpkg.git', vcpkgRoot.path,
    ], workingDirectory: cacheRoot.uri);
    await _run('${vcpkgRoot.path}${Platform.pathSeparator}bootstrap-vcpkg.bat',
      const <String>['-disableMetrics'], workingDirectory: vcpkgRoot.uri);
  }

  final sdkRoot = Directory('${vcpkgRoot.path}${Platform.pathSeparator}installed${Platform.pathSeparator}$triplet');
  final avcodecHeader = File('${sdkRoot.path}${Platform.pathSeparator}include${Platform.pathSeparator}libavcodec${Platform.pathSeparator}avcodec.h');
  final avformatLib = File('${sdkRoot.path}${Platform.pathSeparator}lib${Platform.pathSeparator}avformat.lib');

  if (!await avcodecHeader.exists() || !await avformatLib.exists()) {
    await _run(vcpkg.path, <String>[
      'install', 'ffmpeg[avcodec,avformat,swresample,swscale]:$triplet', '--clean-after-build', '--disable-metrics',
    ], workingDirectory: vcpkgRoot.uri);
  }

  if (!await avcodecHeader.exists() || !await avformatLib.exists()) {
    throw StateError('FFmpeg provisioning completed without the required development SDK at ${sdkRoot.path}.');
  }
...
class _WindowsDependencyProfile {
  const _WindowsDependencyProfile(this.userProfile, this.localAppData, this.appData, this.temp);
...
_WindowsDependencyProfile _windowsDependencyProfile() {
  ...
  final temp = environment['TEMP'] ?? environment['TMP'] ?? Directory.systemTemp.path;
  var userProfile = environment['USERPROFILE'];
  if (userProfile == null || userProfile.isEmpty) {
    final homeDrive = environment['HOMEDRIVE'];
    final homePath = environment['HOMEPATH'];
    if (homeDrive != null && homePath != null && homeDrive.isNotEmpty && homePath.isNotEmpty) {
      userProfile = '$homeDrive$homePath';
    }
  }
  if (userProfile == null || userProfile.isEmpty) {
    userProfile = '${Directory.systemTemp.path}${Platform.pathSeparator}DigitorEngine-profile';
  }

  final localAppData = (environment['LOCALAPPDATA']?.isNotEmpty ?? false)
      ? environment['LOCALAPPDATA']!
      : '$userProfile${Platform.pathSeparator}AppData${Platform.pathSeparator}Local';
...
Future<File?> _findExistingVcpkg() async {
  final candidates = <String>[
    if ((Platform.environment['VCPKG_ROOT']?.isNotEmpty ?? false))
      '${Platform.environment['VCPKG_ROOT']}${Platform.pathSeparator}vcpkg.exe',
    r'C:\vcpkg\vcpkg.exe',
  ];
...
Future<String> _findCmake() async {
  ...
  final sdkRoot = Platform.environment['ANDROID_SDK_ROOT'] ?? Platform.environment['ANDROID_HOME'];
  if (sdkRoot != null && sdkRoot.isNotEmpty) {
    final cmakeRoot = Directory('$sdkRoot${Platform.pathSeparator}cmake');
    if (cmakeRoot.existsSync()) {
      final installs = cmakeRoot.listSync().whereType<Directory>().toList()..sort((a, b) => b.path.compareTo(a.path));
      for (final install in installs) {
        candidates.add('${install.path}${Platform.pathSeparator}bin${Platform.pathSeparator}$executable');
      }
    }
  }
  for (final candidate in candidates) {
...
Future<void> _run(String executable, List<String> arguments, {required Uri workingDirectory}) async {
  final dependencyEnvironment = Platform.isWindows ? _windowsDependencyProfile().environment : null;
