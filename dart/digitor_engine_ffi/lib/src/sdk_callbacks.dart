...
typedef _ExportNative = Int32 Function(
  Pointer<DigitorSdkSessionNative>,
  Pointer<Utf8>,
  Int32,
  Int32,
...
enum DigitorNativeEventKind { seekCompleted, previewCompleted, exportProgress, exportCompleted }
...
final class DigitorSdkCallbackSession {
  ...
  factory DigitorSdkCallbackSession.create({String? libraryPath}) {
    ...
    final create = library.lookupFunction<_CreateNative, _CreateDart>('digitor_sdk_create');
  ...
  final StreamController<DigitorNativeEvent> _events = StreamController.broadcast();
  late final NativeCallable<_CompletionNative> _completion;
  late final NativeCallable<_ProgressNative> _progress;
  DigitorNativeEventKind _pendingCompletion = DigitorNativeEventKind.seekCompleted;
  ...
  void seek(int frame) {
    ...
    final seek = _library.lookupFunction<_SeekNative, _SeekDart>('digitor_sdk_seek_async');
  ...
  void preview({required int frame, required int width, required int height}) {
    ...
    final preview = _library.lookupFunction<_PreviewNative, _PreviewDart>('digitor_sdk_preview_async');
    _check(preview(_handle, frame, width, height, _completion.nativeFunction, nullptr), 'preview');
  }

  void _onCompletion(int result, Pointer<Void> _) {
    if (_disposed) return;
    _events.add(DigitorNativeEvent(kind: _pendingCompletion, resultCode: result));
  }

  void _onProgress(double fraction, int completed, int total, Pointer<Void> _) {
    if (_disposed) return;
    _events.add(DigitorNativeEvent(
  ...
  Future<void> dispose() async {
    if (_disposed) return;
    final destroy = _library.lookupFunction<_DestroyNative, _DestroyDart>('digitor_sdk_destroy');
  ...
  void _ensureAlive() {
    if (_disposed || _handle == nullptr) throw StateError('DigitorSdkCallbackSession is disposed.');
  }
