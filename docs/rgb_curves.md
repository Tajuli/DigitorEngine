...
The backend-neutral `RGB Curves Native` Render Graph pass declares shader-read/source and shader-write/destination dependencies. It accepts only a native executor and has no CPU fallback callback. Vulkan, D3D12, Metal, and GLES produce retained RGBA32F processed-frame resources; live preview consumes those resources directly while numerical validation uses the separate readback entry point.
