...
submissions. Native backends have not yet supplied qualification-owned
destination callbacks, so it is an interface test result, not display or GPU
hardware evidence. The bounded native-pipeline cache contract has deterministic
...
| Registered consumer ownership | N/A | callback pending | callback pending | callback pending | callback pending | acquisition/release tested | `test_render_export` | no readback performed by contract | Interface/design only |
| Bounded pipeline cache | N/A | adoption pending | adoption pending | adoption pending | adoption pending | N/A | `test_render_export` | miss/hit/eviction/invalidation tested | Interface/design only |
